...
| All three Railway production services intentionally source canonical branch | Deployment | VERIFIED | Repeated commits on `feature/spire-ehr-platform`, including `802a8211`, received successful deployment statuses from Sulandra Static Website, `sulandra-website`, and `magnificent-education`. | Continue requiring all affected Railway services green before production-facing work is considered complete. |
...
|---|---|---:|---|---|---|
| Referral/intake creates one durable client/patient identity | Both | PARTIAL | `ClientIntakeCase` stages the case outside the live chart; `upsertPatientFromCase` creates or explicitly links one `SpirePatient`, matches MRN, blocks silent same-name/DOB duplicates with a 409 review requirement, creates `ClientEnrollment`, and approval/promotion is retry-safe and deterministic. `verify-client-intake-automatic-promotion.mjs` verifies approval-triggered promotion wiring. | Exact HHA/DODD intake identity requirements still require authoritative source mapping. | Add database-backed approval → promotion → retry E2E proving exactly one durable patient and no duplicate master records (Issue #145). |
| Demographics, contacts, identifiers, payer/Medicaid data persist | Both | PARTIAL | `SpirePatient`, `SpirePatientContact`, `SpirePatientIdentifier`, and `SpirePayerMemberCoverage` exist. Approval already persists core demographics and phone/email. `CLIENT_INTAKE_MASTER_DATA_NORMALIZATION_V1` now promotes explicit Medicaid/Medicare IDs and structured payer/member identifiers with duplicate-identity conflicts failing closed and without parsing ambiguous insurance free text. | Exact payer/member and program-specific identifier requirements still require authoritative source mapping. | Complete DB-backed verification, confirm patient-address normalization, and verify every intake pathway supplies structured payer/member fields where available (Issue #143/#145). |
| Emergency contacts, guardians/authorized representatives, consent relationships | Both | PARTIAL | Approval persists `SpireEmergencyContact`. Intake captures structured guardian/legal-representative data and immutable typed/witnessed/electronic signatures with signer, relationship, timestamp, IP/user-agent and revocation fields. First-class `SpirePatientProxyRelationship` and `SpireConsentDirective` tables exist. | Exact guardian/consent/e-signature requirements still require program-specific authoritative source verification. | Normalize structured guardian/legal representative and active intake signature evidence into first-class proxy/consent records; do not fabricate identities from POA/payee free text (Issue #144). |
| Admission/start-of-care status and program/company assignment | HHA | PARTIAL | Home Health referral/intake is company-scoped. `HomeHealthEpisode` supports PRE_ADMISSION/SOC_READY/ACTIVE/ON_HOLD/DISCHARGED/CANCELLED, requested and actual SOC dates, care plans/orders/visits and append-only events. Start-of-care routes require selected Home Health entity, approved intake, operational readiness and clinical prerequisites before activation. | CMS/ODM/OAC source verification pending. | Add DB-backed HHA intake → episode → SOC-ready → activation regression and map each gate to authoritative HHA requirements (Issue #145). |
| DODD individual/provider/service-home linkage | DODD | PARTIAL | Company-aware `ClientEnrollment`, `SpireHome`, `SpirePatientHomeAssignment`, service-home/client assignment and service-authorization structures exist. SCLS residential/service-home APIs validate selected company/enrollment before placement and prevent cross-company assignment. Intake promotion creates coded service authorizations only when code, valid dates and positive authorized units are present. | DODD/OAC source verification pending. | Add DB-backed SCLS intake → enrollment → authorization → explicit home placement regression and verify ISP/service-plan handoff before promoting to VERIFIED (Issue #145). |

### Section A closure work

- Issue #143 — normalize and verify payer/member identifiers and first-class coverage.
- Issue #144 — normalize guardian/legal-representative and consent evidence.
- Issue #145 — add database-backed intake-to-Spire, HHA SOC, and SCLS placement/authorization E2E regressions.
- Determine and verify the canonical first-class patient-address model; do not rely only on the intake JSON or admission summary.
- Section A remains PARTIAL until database-backed E2E evidence and authoritative program-specific source mapping are complete.

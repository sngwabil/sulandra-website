  ...
  app.post('/api/auth/forgot-password', async (req, res, next) => {
    try {
      ...
      if (user) {
        ...
        const hrEmail = process.env.CAREERS_EMAIL_FROM || process.env.SMTP_FROM || process.env.SMTP_USER || '';
        const recipients = [personalEmail, employeeEmail, hrEmail];
        const safeUsername = escapeHtml(employeeUsername(user));
        const safeReference = escapeHtml(resetReference);
        ...
        await sendMail(
          ...
          `<p>A password recovery request was received for <strong>${safeUsername}</strong>.</p><p>For security, Sulandra never emails an existing password. Contact Human Resources and provide recovery reference <strong>${safeReference}</strong> to receive a temporary password or secure reset link.</p><p>If you did not request this, contact Human Resources immediately.</p><p><strong>Human Resources</strong><br>Sulandra Health</p>`,
          text,
        );
      }
    ...
    try {
      ...
      if (user) {
        ...
        await sendMail(
          ...
          `<p>Your Sulandra employee username is:</p><p style="font-size:18px"><strong>${safeUsername}</strong></p><p>Employee usernames use the first letter of the first name plus the full last name followed by <strong>@sulandrahealth.com</strong>.</p><p><strong>Human Resources</strong><br>Sulandra Health</p>`,

...
export function registerApplicantTerminalRoutes(app: express.Express, prisma: PrismaClient, helpers: Helpers) {
  ...
  app.post('/api/admin/applications/:id/reject', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
  ...
  app.post('/api/admin/applications/:id/archive', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);
      const input = terminalActionInput.parse(req.body);
      const applicationId = String(req.params.id);
  ...
  app.post('/api/admin/applications/:id/restore', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);

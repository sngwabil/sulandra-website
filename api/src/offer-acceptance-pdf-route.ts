...
function dateText(value: unknown) {
  ...
  return Number.isNaN(date.getTime()) ? 'To be confirmed' : date.toLocaleDateString('en-US');
}

function money(value: unknown, type: unknown) {
  const amount = Number(value || 0);
  const formatted = amount.toLocaleString('en-US', { style: 'currency', currency: 'USD' });
  return String(type) === 'SALARY' ? `${formatted} per year` : `${formatted} per hour`;
}

function buildSignedOfferPdf(offer: any, acceptedBy: string, signature: string, acceptedAt: Date) {
  ...
  add('SULANDRA HEALTH', true);
  add('Sulandra Community Living Services', true);
  add('A Division of Sulandra Health');
  add('');
  add('OFFER OF EMPLOYMENT', true);
  add('');
  add(`Applicant: ${offer.firstName || ''} ${offer.lastName || ''}`);
  add(`Position: ${offer.positionTitle || 'As stated in the offer'}`);
  add(`Department: ${offer.department || 'As assigned'}`);
  add(`Employment type: ${String(offer.employmentType || '').replaceAll('_', ' ')}`);
  add(`Compensation: ${money(offer.payAmount, offer.compensationType)}`);
  add(`Expected shift: ${offer.shift || 'As scheduled'}`);
  add(`Supervisor: ${offer.supervisorName || 'To be assigned'}`);
  add(`Work location: ${offer.workLocation || 'To be confirmed'}`);
  add(`Anticipated start date: ${dateText(offer.startDate)}`);
  add(`Orientation date: ${dateText(offer.orientationDate)}`);
  add(`Introductory period: ${offer.probationDays ?? 90} days`);
  add(`PTO eligibility: ${offer.ptoEligible ? 'Eligible' : 'Not currently eligible'}`);
  add(`Benefits eligibility: ${offer.benefitsEligible ? 'Eligible' : 'Not currently eligible'}`);
  if (offer.bonusAmount) add(`Bonus: ${money(offer.bonusAmount, 'ONE_TIME')}`);
  add('');
  ...
  add('ELECTRONIC ACCEPTANCE', true);
  add(`Accepted by: ${acceptedBy}`);
  add(`Electronic signature: ${signature}`);
  add(`Accepted on: ${acceptedAt.toLocaleString('en-US')}`);
  add('');
  ...
  add('Sulandra Human Resources Department', true);
  add('Sulandra Community Living Services');
  add('A Division of Sulandra Health');
  ...
  pages.forEach((pageLines) => {
    ...
    const commands: string[] = ['BT', '/F2 15 Tf', '54 756 Td', '18 TL'];
...
export function registerOfferAcceptancePdfRoute(
  app: express.Express,
  prisma: PrismaClient,
  helpers: Helpers,
) {
  app.post('/public/careers/offers/:token/accept', async (req, res, next) => {
    try {
      ...
      if (offer.status === 'OFFER_ACCEPTED' || offer.acceptedAt) {
        return res.json({
          data: {
            ...
            message: 'Your signed Offer of Employment has already been received by the Sulandra Human Resources Department.',
          },
        });
      }

      const acceptedAt = new Date();
      const pdf = buildSignedOfferPdf(offer, input.fullLegalName, input.signature, acceptedAt);
      const pdfHash = createHash('sha256').update(pdf).digest('hex');
      const documentId = randomUUID();
      const fileName = `Signed-Offer-of-Employment-${cleanPdfText(`${offer.firstName}-${offer.lastName}`).replaceAll(' ', '-')}.pdf`;

      await prisma.$transaction(async (tx) => {
        await tx.$executeRawUnsafe(
          `UPDATE "EmploymentOffer"
              SET "status"='OFFER_ACCEPTED',"acceptedAt"=$1,
                  "acceptedByName"=$2,"signature"=$3,"updatedAt"=NOW()
            WHERE "id"=$4`,
          acceptedAt,
          input.fullLegalName,
          input.signature,
        ...
        await tx.$executeRawUnsafe(
          `DELETE FROM "ApplicantDocument"
            WHERE "applicationId"=$1 AND "label"='Signed Offer of Employment'`,
          offer.applicationId,
        );
        await tx.$executeRawUnsafe(
          `INSERT INTO "ApplicantDocument"
            ("id","applicationId","category","label","status","fileName","mimeType",
             "sizeBytes","fileData","contentSha256","uploadedByType","uploadedAt","createdAt","updatedAt")
      ...
      await helpers.audit({}, 'ACCEPT_EMPLOYMENT_OFFER', 'EmploymentOffer', offer.id, {
        applicationId: offer.applicationId,
        signedOfferDocumentId: documentId,
      ...
      res.json({
        data: {
          status: 'OFFER_ACCEPTED',
          signedOfferDocumentId: documentId,
          message: 'Your signed Offer of Employment has been received by the Sulandra Human Resources Department.',
        },
      });
    } catch (error) {
      next(error);

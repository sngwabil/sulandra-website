...
export function evaluateMedicaidRuleEnvelope(input: MedicaidRuleEnvelopeInput): MedicaidRuleEnvelopeDecision {
  ...
  if (bool(config.requirePositiveUnits, true) && (eventUnits === null || eventUnits <= 0)) blockers.push('Billable Medicaid units must be greater than zero.');
  ...
  if (rule) {
    ...
    if (maxReviewAgeDays !== null && maxReviewAgeDays > 0 && dateKey(rule.reviewedOn)) {
      ...
      if (Number.isFinite(reviewed) && Number.isFinite(at) && at - reviewed > maxReviewAgeDays * 86400000) warnings.push(`Active Medicaid rule version was last reviewed more than ${maxReviewAgeDays} days before the service date.`);
    }
  }

  const evidence = {
    engine: 'SPIRE_MEDICAID_RULE_ENVELOPE_1_1', serviceFamily, serviceCode, serviceDate,
    eventUnits, unitType: unitType || null, payer: payer || null, payerProgram: payerProgram || null, waiverType: waiverType || null,
    authorization: authorization ? { id: authorization.id ?? null, status: authorization.status ?? null, serviceCode: authorization.serviceCode ?? null, startDate: authorization.startDate ?? null, endDate: authorization.endDate ?? null, authorizedUnits, deliveredUnits, remainingUnits } : null,
    rule: rule ? { id: rule.id ?? null, ruleCode: rule.ruleCode ?? null, version: rule.version ?? null, scope: rule.scope ?? null, effectiveFrom: rule.effectiveFrom ?? null, effectiveTo: rule.effectiveTo ?? null, authority: rule.authority ?? null, reviewedOn: rule.reviewedOn ?? null, unitMethod: rule.unitMethod ?? null } : null,
    configuredRestrictions: { allowedPayers: [...allowedPayers], allowedPayerPrograms: [...allowedPrograms], allowedWaiverTypes: [...allowedWaivers], allowedServiceCodes: [...allowedServiceCodes], allowedUnitTypes: [...allowedUnitTypes], minUnitsPerEvent: minUnits, maxUnitsPerEvent: maxUnits },
  };

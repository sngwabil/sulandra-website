  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const base = configuredBase && !configuredBase.endsWith('/offer-acceptance.html')
          ? configuredBase
          : 'https://www.sulandrahealth.com/offer-packet.html';
        ...
        const orientationDate = input.orientationDate ? input.orientationDate.toLocaleDateString('en-US') : 'To be confirmed';
        ...
        const emailHtml = `
          <div style="font-family:Segoe UI,Arial,sans-serif;color:#183153;line-height:1.6;max-width:760px;margin:0 auto;border:1px solid #d8e3ed;border-radius:14px;overflow:hidden;background:#ffffff">
            ...
            <div style="padding:30px">
              ...
              <p style="margin:26px 0;text-align:center"><a href="${offerUrl}" style="display:inline-block;background:#087fb8;color:#ffffff;text-decoration:none;padding:14px 24px;border-radius:8px;font-weight:700">Open Secure Onboarding Packet</a></p>
        ...
        try {
          deliveryStatus = await sendOfferEmail(application.email, `Offer of Employment — ${input.positionTitle}`, emailHtml, emailText);
        } catch (mailError) {
        ...
        res.status(201).json({
          data: { offerId, status: 'OFFER_SENT', offerUrl, requiredDocuments: input.requiredDocuments, deliveryStatus },
        });
      } catch (error) {
        next(error);
      }

...
type Input={auth:AuthContext;actionId:string;actionType:RoutineActionType;payload:Json|string};
...
export const isRoutineITAgentAction=(value:string):value is RoutineActionType=>[
  'PUBLISH_INTRAnet_CONTENT','GENERATE_INTRAnet_MEME','SEND_ANNOUNCEMENT','SEND_NOTIFICATION','SEND_EMAIL',
].includes(value);
...
export async function executeRoutineITAgentAction(prisma:PrismaClient,input:Input){
  ...
  }else throw httpError(409,`Action ${input.actionType} is not a routine IT Specialist operation.`);

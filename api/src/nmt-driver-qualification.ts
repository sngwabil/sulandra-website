...
export async function evaluateNmtDriverEligibility(prisma:PrismaClient,input:{organizationId:string;legalEntityId:string;driverId:string;serviceDate:Date}):Promise<NmtDriverEligibility>{
  ...
  const profiles=await prisma.$queryRawUnsafe<DriverProfile[]>(`SELECT "id","active","userId","displayName" FROM "NmtDriverProfile" WHERE "organizationId"=$1 AND "legalEntityId"=$2 AND "id"=$3 LIMIT 1`,input.organizationId,input.legalEntityId,input.driverId);

...
app.use(['/api/auth/forgot-password', '/api/auth/forgot-username'], rateLimit({
  windowMs: 15 * 60 * 1_000,
  limit: 5,
  standardHeaders: 'draft-8',
  legacyHeaders: false,
...
app.use('/public/consultations', rateLimit({
  windowMs: 15 * 60 * 1_000,
  limit: 5,
  standardHeaders: 'draft-8',
  legacyHeaders: false,
...
app.use('/api/admin/desktop-profile',
...
const audit = async (
  auth: Partial<Omit<AuthContext, 'role'>> & { role?: unknown },
  action: string,
  resourceType: string,
  resourceId?: string,
  metadata?: object,
...
// ADMIN_DESKTOP_PROFILE_ROUTES_V1

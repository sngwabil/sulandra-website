...
const applicantRole = z.enum(['DSP', 'LPN', 'RN', 'DELEGATING_NURSE', 'DRIVER', 'GENERAL', 'DOO']);
...
function roleForOpening(title: string, department?: string | null): ApplicantRole {
  ...
  if (/\bcoo\b|chief operating officer/.test(value)) return 'DOO';
...
function applicationPathForOpening(row: any) {
  ...
  if (role === 'DOO') return `/applycoo.html?opening=${encodeURIComponent(row.slug)}`;
...
export function registerCareersRoutes(
  ...
  app.post('/public/careers/applications', async (req, res, next) => {
    ...
    try {
      ...
      await prisma.$executeRawUnsafe(
        ...
        JSON.stringify(routedApplicationData),
      );
  ...
  app.get('/api/admin/job-openings', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (_req, res, next) => {
    try {
  ...
  app.post('/api/admin/job-openings', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);
  ...
  app.patch('/api/admin/job-openings/:id', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);
  ...
  app.get('/api/admin/applications', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);
  ...
  app.get('/api/admin/applications/:id/folder', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);
      const id = String(req.params.id);
  ...
  app.post('/api/admin/applications/:id/request-document', requireRoles(UserRole.ADMINISTRATOR, UserRole.DOO), async (req, res, next) => {
    try {
      const auth = authOf(res);
      const id = String(req.params.id);

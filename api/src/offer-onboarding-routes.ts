...
const documentCompleteSchema = z.object({ fullLegalName: z.string().trim().min(2).max(180), signature: z.string().trim().min(2).max(500) });
const acceptSchema = z.object({ fullLegalName: z.string().trim().min(2).max(180), signature: z.string().trim().min(2).max(500), acceptedTerms: z.literal(true) });
const DEFAULT_DOCUMENTS = ['Offer Letter','Form W-4','Form I-9','Direct Deposit Authorization','Confidentiality Agreement','HIPAA Acknowledgment','Non-Disclosure Agreement','Employee Handbook Acknowledgment','Drug-Free Workplace Policy','Background Check Authorization','Emergency Contact Form','Technology Acceptable Use Policy'];
...
async function documentProgress(prisma: PrismaClient, offerId: string) {
  const documents = await prisma.$queryRawUnsafe<any[]>(`SELECT "id","name","status","signedByName","completedAt" FROM "EmploymentOfferDocument" WHERE "offerId"=$1 ORDER BY "createdAt","name"`, offerId);

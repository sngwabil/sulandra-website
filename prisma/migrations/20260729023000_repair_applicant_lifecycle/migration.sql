-- Repair the applicant lifecycle schema on environments where the original
-- migration was recorded but one or more lifecycle objects were not created.
-- Every operation is intentionally idempotent so this migration is safe on
-- healthy environments as well as partially migrated Railway databases.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'EmployeeApplication_preferredCommunication_check'
  ) THEN
    ALTER TABLE "EmployeeApplication"

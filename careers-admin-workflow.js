(function (global) {
  ...
  function mount(options) {
    ...
    function closeOfferModal() {
      document.querySelector("[data-scw-offer-modal]")?.remove();
    }
    ...
    function openOfferModal(application) {
      ...
      wrapper.innerHTML = `<section class="scw-modal" role="dialog" aria-modal="true" aria-labelledby="scw-offer-title">
        <h2 id="scw-offer-title">Prepare Employment Offer</h2>
        <p>Complete the employment terms for ${esc(application.firstName)} ${esc(application.lastName)}. No employee profile will be created at this step.</p>
        <div class="scw-modal-grid">
          <div class="scw-field"><label>Position title</label><input data-offer-position value="${esc(application.positionTitle || application.jobTitle || application.role || "")}"></div>
          <div class="scw-field"><label>Department</label><input data-offer-department></div>
          <div class="scw-field"><label>Supervisor</label><input data-offer-supervisor></div>
          <div class="scw-field"><label>Employment type</label><select data-offer-employment-type><option value="FULL_TIME">Full-time</option><option value="PART_TIME">Part-time</option><option value="PRN">PRN</option><option value="CONTRACT">Contract</option></select></div>
          <div class="scw-field"><label>Compensation type</label><select data-offer-compensation-type><option value="HOURLY">Hourly</option><option value="SALARY">Salary</option></select></div>
          <div class="scw-field"><label>Pay amount</label><input data-offer-pay type="number" min="0.01" step="0.01" placeholder="e.g. 18.50"></div>
          <div class="scw-field"><label>Shift</label><input data-offer-shift></div>
          <div class="scw-field"><label>Start date</label><input data-offer-start type="date"></div>
          <div class="scw-field"><label>Orientation date</label><input data-offer-orientation type="date"></div>
          <div class="scw-field"><label>Work location</label><input data-offer-location></div>
          <div class="scw-field"><label>Probation period (days)</label><input data-offer-probation type="number" min="0" max="365" value="90"></div>
          <div class="scw-field"><label>Bonus amount</label><input data-offer-bonus type="number" min="0" step="0.01"></div>
        </div>
        <label class="scw-check"><input type="checkbox" data-offer-pto> PTO eligible</label>
        <label class="scw-check"><input type="checkbox" data-offer-benefits> Benefits eligible</label>
        <div class="scw-field"><label>Offer notes</label><textarea data-offer-notes></textarea></div>
        <div class="scw-field"><label>Required disclosures and onboarding paperwork</label><div class="scw-offer-docs">${DEFAULT_OFFER_DOCUMENTS.map((name) => `<label><input type="checkbox" data-offer-document value="${esc(name)}" checked>${esc(name)}</label>`).join("")}</div></div>
        <div data-offer-error hidden></div>
        <div class="scw-modal-actions"><button type="button" class="scw-button secondary" data-offer-cancel>Cancel</button><button type="button" class="scw-button" data-offer-send>Send Offer</button></div>
      </section>`;
      ...
      wrapper.querySelector("[data-offer-send]").addEventListener("click", async () => {
        ...
        try {
          ...
          await client.request(`/api/admin/applications/${encodeURIComponent(options.applicationId)}/offers`, {
            method: "POST",
    ...
    async function saveStatus() {
      ...
      try {
        if (status === "HIRED") {
          openOfferModal(state.folder.application || state.folder);
          return;
        }

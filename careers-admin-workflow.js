(function (global) {
  ...
  function mount(options) {
    ...
    async function requestDocument() {
      ...
      try {
        setBusy(true);
        const result = await client.request(`/api/admin/applications/${encodeURIComponent(options.applicationId)}/request-document`, {
          method: "POST",
        ...
        showMessage(
          delivery === "SENT"
            ? "Document request added and the email was sent."
            : delivery === "FAILED"
              ? "Document request added, but the email could not be sent."
              : "Document request added. Email delivery is waiting for configuration.",

...
if(!payroll.includes('if (value instanceof Date) return value.toISOString();\n  if (Array.isArray(value))')&&payroll.includes(stableOld)) payroll=payroll.replace(stableOld,stableNew);
payroll=payroll.replaceAll(`RETURN CASE WHEN TG_OP='DELETE' THEN OLD ELSE NEW END;`,`IF TG_OP='DELETE' THEN RETURN OLD; END IF;\n      RETURN NEW;`);

...
requireMarkers(compositor,[
  'evaluateRevenuePrebillBatch',
  'assertRevenuePrebillBatch',
  'evaluateSpireEvvPrebill',
  'evaluateSpireDoddBilling',
  'recordSpireEvvPrebillDecision',
  'recordSpireDoddBillingDecision',
  'REVENUE_PREBILL_HARD_STOP_BLOCKED',
  'REVENUE_PREBILL_SNAPSHOT_STALE',
  "createHash('sha256')",
  'Training-only service events cannot be billed.',
  'Service event is not marked billable.',
], 'spire-prebill-hard-stop.ts');

requireMarkers(revenue,[
  "from './spire-prebill-hard-stop.js'",
  "stage:'BATCH_FINALIZE'",
  "stage:'CSV_EXPORT'",
], 'revenue-cycle-routes.ts');

requireMarkers(exchange,[
  "from './spire-prebill-hard-stop.js'",
  "stage:'CLAIM_GENERATE'",
  "stage:'CLAIM_DOWNLOAD'",
  "stage:'CLAIM_HANDOFF'",
  'prebillHardStop:{fingerprint:prebill.fingerprint',
  'REVENUE_PREBILL_SNAPSHOT_MISSING',
], 'revenue-cycle-claim-exchange-routes.ts');
...
if(failures.length){
  console.error('Prebill hard-stop verification failed:\n- '+failures.join('\n- '));
  process.exit(1);
}

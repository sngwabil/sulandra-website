...
async function patchAdminContext(target) {
  ...
  if (source.includes(settingsMarker)) source = source.replace(settingsMarker, chroniclesMarker);
  if (!source.includes(chroniclesMarker)) throw new Error(`Company Chronicles navigation marker missing in ${path.relative(root, target)}`);
...
for (const marker of [chroniclesMarker, runtimeSrc, adminSrc]) {
...
console.log('Company Chronicles published: global white-label runtime is present on non-Admin HTML, canonical Admin navigation exposes Company Chronicles, and branding assets are loaded by the Admin shell.');

...
for(const asset of ['it-agent-ui-regression-repair.css','it-agent-status-board-split.css','it-agent-ipad-load-guard.css','it-agent-enterprise-shell.css','it-agent-header-polish.css','it-agent-dark-conversation.css','it-agent-real-terminal.css']){
  const escaped=asset.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  html=html.replace(new RegExp(`\\s*<link rel="stylesheet" href="/assets/${escaped}(?:\\?v=[^"']+)?">\\s*`,'g'),'\n');
}
for(const asset of ['it-agent-ui-regression-repair.js','it-agent-status-board-finalizer.js','it-agent-ipad-load-guard.js','it-agent-enterprise-shell.js','it-agent-header-polish.js','it-agent-sidebar-persistence.js','it-agent-real-terminal.js']){
  const escaped=asset.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
...
html=html.replace('</head>',`${ipadPreboot}${tags.repairCss}${tags.statusCss}${tags.ipadCss}${tags.enterpriseCss}${tags.headerCss}${tags.darkCss}${tags.realTerminalCss}</head>`);
html=html.replace('</body>',`${tags.repairJs}${tags.statusJs}${tags.ipadJs}${tags.enterpriseJs}${tags.headerJs}${tags.sidebarJs}${tags.realTerminalJs}</body>`);

for(const marker of ['Sulandra IT Agent','Ask IT Agent','/assets/it-agent-chatgpt-workspace.css','/assets/it-agent-chatgpt-workspace.js','/assets/it-agent-conversational-ui.css','/assets/it-agent-conversational-ui.js','/assets/it-agent-action-center-tab.js','/assets/it-agent-ui-regression-repair.css','/assets/it-agent-ui-regression-repair.js','/assets/it-agent-status-board-split.css','/assets/it-agent-status-board-finalizer.js','/assets/it-agent-ipad-load-guard.css','/assets/it-agent-ipad-load-guard.js','/assets/it-agent-enterprise-shell.css','/assets/it-agent-enterprise-shell.js','/assets/it-agent-header-polish.css','/assets/it-agent-header-polish.js','/assets/it-agent-dark-conversation.css','/assets/it-agent-sidebar-persistence.js','/assets/it-agent-real-terminal.css','/assets/it-agent-real-terminal.js','itws-preboot-critical','itws-preboot-script']){
...
for(const [label,file] of [['regression repair',paths.repairJs],['Action Center',paths.actionCenter],['Status Board finalizer',paths.statusJs],['iPad load guard',paths.ipadJs],['engineering shell',paths.enterpriseJs],['header polish',paths.headerJs],['sidebar persistence',paths.sidebarJs],['real terminal',paths.realTerminalJs]]){
...
console.log('IT Solutions engineering workspace preserved: aligned navy chat, explicit left-rail state, separate Status Board/Action Center, Tell Sulandra workflow, and the isolated multi-session Real Terminal are published together.');

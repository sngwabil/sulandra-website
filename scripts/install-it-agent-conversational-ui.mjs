...
const statusBoardSplitCssTag='<link rel="stylesheet" href="/assets/it-agent-status-board-split.css?v=20260829-status-board-split-2">';
const statusBoardFinalizerTag='<script src="/assets/it-agent-status-board-finalizer.js?v=20260829-status-board-3"></script>';
...
// Status Board is a dedicated per-chat observable working trace in a docked rail.
...
for(const marker of ['IT_AGENT_STATUS_BOARD_SPLIT_V2','itws-status-board-open','--itws-status-board-width','itws-status-board-feed','data-itws-status-board-ready']){
  if(!statusBoardSplitCss.includes(marker))throw new Error(`IT Agent Status Board split layout missing ${marker}`);
}
for(const marker of ['IT_AGENT_STATUS_BOARD_FINALIZER_V3','itwsStatusBoardFeed','syncFeed','OPEN_KEY','private model reasoning','Status Board']){
...
console.log('IT Agent current chat-first workspace preserved: Action Center is separate under Operations; Status Board is a persistent per-chat working trace; inline artifacts and iPad composer repairs remain active.');

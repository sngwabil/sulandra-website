...
await Promise.all([access(portalPath),access(cssPath),access(jsPath),access(actionCenterPath),access(repairCssPath),access(repairJsPath),access(statusBoardSplitCssPath),access(statusBoardFinalizerPath),access(ipadGuardCssPath),access(ipadGuardJsPath)]);
...
html=html.replace('</head>',`${ipadPreboot}${repairCssTag}${statusBoardSplitCssTag}${ipadGuardCssTag}</head>`);
html=html.replace('</body>',`${repairJsTag}${statusBoardFinalizerTag}${ipadGuardJsTag}</body>`);

for(const marker of ['Sulandra IT Agent','Ask IT Agent','/assets/it-agent-chatgpt-workspace.css','/assets/it-agent-chatgpt-workspace.js','/assets/it-agent-conversational-ui.css','/assets/it-agent-conversational-ui.js','/assets/it-agent-action-center-tab.js','/assets/it-agent-ui-regression-repair.css','/assets/it-agent-ui-regression-repair.js','/assets/it-agent-status-board-split.css','/assets/it-agent-status-board-finalizer.js','/assets/it-agent-ipad-load-guard.css','/assets/it-agent-ipad-load-guard.js','itws-preboot-critical','itws-preboot-script']){
  if(!html.includes(marker))throw new Error(`IT Agent current chat-first UI missing ${marker}`);
}
const [actionCenter,repairCss,repairJs,statusBoardSplitCss,statusBoardFinalizer,ipadGuardCss,ipadGuardJs]=await Promise.all([readFile(actionCenterPath,'utf8'),readFile(repairCssPath,'utf8'),readFile(repairJsPath,'utf8'),readFile(statusBoardSplitCssPath,'utf8'),readFile(statusBoardFinalizerPath,'utf8'),readFile(ipadGuardCssPath,'utf8'),readFile(ipadGuardJsPath,'utf8')]);

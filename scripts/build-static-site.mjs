...
for (const marker of ['<html','<head','<body','</html>',"window.SULANDRA_API_BASE='https://sulandra-website-production-5fc4.up.railway.app'",'/assets/sulandra-entity-context.js','SPIRE_MASTER_DEFECT_FIXES_V1','SPIRE_MAIN_TAB_ICONS_V1']) if(!publishedSpireMaster.includes(marker)) throw new Error(`Static publication regression: standalone SPIRE chart missing ${marker}`);

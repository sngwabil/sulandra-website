...
const read = async relative => {
  ...
  catch { failures.push(`Missing canonical source: ${relative}`); return ''; }
};
const readPublished = async relative => {
...
const forbid = (source, markers, label) => {
  for (const marker of markers) if (source.includes(marker)) failures.push(`${label} still references ${marker}`);
};

const [adminSource, adminPublished, context, shellJs, shellCss, buildScript, packageJson, serviceRequestPublisher] = await Promise.all([
  read('admin.html'),
  readPublished('admin.html'),
  read('assets/admin-company-context.js'),
  read('assets/admin-shell.js'),
  read('assets/admin-shell.css'),
...
for (const [label, source] of [
  ['Admin company/navigation context', context],
  ['Admin shell runtime', shellJs],
]) {
  try { new Function(source); }
  catch (error) { failures.push(`${label} has JavaScript syntax error: ${error instanceof Error ? error.message : String(error)}`); }
}
...
requireMarkers(context, [
  ...
  'data-company-module',
  'renderRightDrawer',
  'window.SulandraAdminNavigation',
...
forbid(buildScript, [
  "restore-modern-admin-portal.mjs",
  "finalize-admin-fullscreen-layout.mjs",
  "install-employee-management-frontend.mjs",
  "admin-achieved-archive-fix.js?v=20260808-achieved-archive-1\"></script>",
  "fix-admin-company-settings-backend.mjs",
], 'Static build');
requireMarkers(buildScript, [
...
forbid(packageJson, [
  'scripts/fix-admin-time-attendance-link.mjs',
  'scripts/restore-modern-admin-portal.mjs',
  'scripts/install-employee-management-frontend.mjs',
  'scripts/finalize-admin-fullscreen-layout.mjs',
...
for (const relative of ['assets/admin-shell.css','assets/admin-shell.js','assets/admin-company-context.js']) {
  try { await stat(path.join(dist, relative)); }
  catch { failures.push(`Canonical Admin publication missing ${relative}`); }
}

if (failures.length) {
  console.error('Canonical Admin source verification failed:\n- ' + failures.join('\n- '));
  process.exit(1);
}
console.log('Canonical Admin source verified: one navigation registry owns top/left/company-specific/portal routes, the modern shell is source-controlled, and the static build no longer manufactures Admin through post-build mutation scripts.');

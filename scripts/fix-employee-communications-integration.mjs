...
if(!frontend.includes("sulandra:employee:access-token")) throw new Error('Employee communications is not using canonical Admin authentication');
if(!frontend.includes("window.SulandraCompanyContext?.headers?.()")) throw new Error('Employee communications is not carrying selected-company context');
await writeFile(frontendPath,frontend,'utf8');

console.log('Employee 360 communications integration is build-safe and now uses canonical Admin authentication/company scope without replacing the Employees module.');

...
await Promise.all([stat(launchpadPath),stat(launcherPath),stat(adminPath),stat(contextPath)]);
const [launchpad,launcher,admin,context] = await Promise.all([
  readFile(launchpadPath,'utf8'),readFile(launcherPath,'utf8'),readFile(adminPath,'utf8'),readFile(contextPath,'utf8'),
]);
...
if (!context.includes("'/assets/admin-enterprise-apps-launcher.js?v=20260810-enterprise-apps-1'")) throw new Error('Enterprise Apps regression: canonical Admin bootstrap does not load the launcher');
if (admin.includes('/assets/admin-enterprise-apps-launcher.js?v=20260810-enterprise-apps-1')) throw new Error('Enterprise Apps regression: launcher is duplicated by direct post-build Admin injection');
...
console.log(`Enterprise Apps verified: ${requiredAppIds.length} required apps, ${checked.size} internal targets, company-aware live status, canonical Admin launcher ownership and published target pages.`);

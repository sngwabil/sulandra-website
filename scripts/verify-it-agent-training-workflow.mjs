...
const [migration,campaigns,careers,workbench]=await Promise.all([
...
try{
  ...
  await access(path.join(root,'education-campaign.html'));
  const [portal,reviewPage]=await Promise.all([read('it-solutions.html'),read('education-campaign.html')]);
  ...
  console.log('IT Agent education frontend verification skipped because frontend sources are not present in this API-only build image.');
}

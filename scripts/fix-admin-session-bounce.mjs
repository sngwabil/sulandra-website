...
for (const relative of files) {
  ...
  source = source.replaceAll(staleApi, canonicalApi).replaceAll(productionApi, canonicalApi);
  ...
  if (source.includes('if (response.status === 401) signOut();')) throw new Error(`${relative} still destroys the global Sulandra session on a feature-level 401.`);
  if (!source.includes('role !== "ADMINISTRATOR"') || !source.includes('role === "DOO" ? "doo.html"')) throw new Error(`${relative} does not keep owner admin.html separate from CEO/DOO role workspaces.`);
  if (!source.includes('ADMIN_TOKEN_KEY') || !source.includes('ADMIN_SESSION_KEY')) throw new Error(`${relative} does not prefer the independent Admin tab session.`);
  if (!source.includes('admin-login.html?returnTo=/admin-operations.html')) throw new Error(`${relative} does not return missing Operations sessions to Admin sign-in.`);
  if (!source.includes('"PROGRAM_MANAGER", "HR_MANAGER"')) throw new Error(`${relative} does not allow Program Manager into the company Operations admin workspace.`);
  if (!source.includes('data-sulandra-admin-session-security')) throw new Error(`${relative} does not load the privileged Admin session security guard.`);
  if (!source.includes(canonicalApi)) throw new Error(`${relative} is not using the configured Railway API.`);
  await writeFile(target, source, 'utf8');
}

await import('./install-admin-command-center-live-fix.mjs');

console.log(`Admin authentication separated from Employee Portal and verified for ${canonicalApi}: independent Admin tab keys are preferred, missing sessions return to Admin sign-in, Program Manager can use company Operations, and owner-only admin.html plus privileged inactivity security remain enforced.`);
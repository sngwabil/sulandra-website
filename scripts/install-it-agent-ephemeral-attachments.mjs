...
const artifactCallTrusted="const attachmentParts=await buildITAgentAttachmentContent(prisma,{organizationId:auth.organizationId,conversationId:conversationId as string,artifactIds:input.attachmentIds});const knowledge=await knowledgeFor(input.message);const payload=await askOpenAI(history,await context(auth,knowledge,conversationId as string),attachmentParts);";
const artifactCallLegacy="const attachmentParts=await buildITAgentAttachmentContent(prisma,{organizationId:auth.organizationId,conversationId:conversationId as string,artifactIds:input.attachmentIds});const knowledge=await knowledgeFor(input.message);const payload=await askOpenAI(history,await context(auth,knowledge),attachmentParts);";
const ephemeralCall="const [storedAttachmentParts,ephemeralAttachmentParts]=await Promise.all([buildITAgentAttachmentContent(prisma,{organizationId:auth.organizationId,conversationId:conversationId as string,artifactIds:input.attachmentIds}),buildITAgentEphemeralAttachmentContent({attachments:input.attachments})]);const attachmentParts=[...storedAttachmentParts,...ephemeralAttachmentParts];const knowledge=await knowledgeFor(input.message);const payload=await askOpenAI(history,await context(auth,knowledge,conversationId as string),attachmentParts);";
if(workbench.includes(artifactCallTrusted))workbench=workbench.replace(artifactCallTrusted,ephemeralCall);
else if(workbench.includes(artifactCallLegacy))workbench=workbench.replace(artifactCallLegacy,ephemeralCall);

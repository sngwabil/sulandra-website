...
const [installer,preparer,round7,round8,round9,round10,payroll,test,config,workflow,pkg,interview,applicant,offer,workforceAdvanced,taskPage,navGuard,deepLink,hhTokenBootstrap,spirePage,employeePortal]=await Promise.all([
  read('scripts/install-business-path-uat-bridges.mjs'),read('scripts/prepare-production-business-uat.mjs'),read('scripts/prepare-production-business-uat-round7.mjs'),read('scripts/prepare-production-business-uat-round8.mjs'),read('scripts/prepare-production-business-uat-round9.mjs'),read('scripts/prepare-production-business-uat-round10.mjs'),read('assets/workforce-payroll-readiness.js'),read('tests/production-business-path-uat.spec.mjs'),read('playwright.business-uat.config.mjs'),read('.github/workflows/production-business-uat.yml'),read('package.json'),read('interview-admin-scheduler.js'),read('applicant-portal.html'),read('offer-acceptance.html'),read('api/src/workforce-advanced-routes.ts'),read('scls-tasks.html'),read('assets/employee-role-navigation-guard.js'),read('assets/spire-deep-link.js'),read('assets/home-health-referral-token-bootstrap.js'),read('spire.html'),read('employee-portal.html')
]);
...
expect(deepLink.includes('waitForAuthorizedPatient')&&deepLink.includes("query.get('patientId')")&&deepLink.includes("hash.get('tab')")&&deepLink.includes('fallbackOpenRequestedChart')&&deepLink.includes('directOpenRequestedChart')&&deepLink.includes('BUSINESS_UAT_DIRECT_CHART_OPEN')&&deepLink.includes('BUSINESS_UAT_PATIENT_CLICK_RECOVERY')&&deepLink.includes('SpireOpenPatient'),'SPIRE patient/tab direct, fallback, and visible-row recovery bridge is incomplete');
expect(spirePage.includes('spire-app-v2.js?')&&spirePage.includes('spire-deep-link.js?'),'SPIRE page source is missing versioned application/deep-link assets');
...
for(const rel of ['assets/workforce-payroll-readiness.js','assets/employee-role-navigation-guard.js','assets/spire-deep-link.js','assets/home-health-referral-token-bootstrap.js','scripts/install-business-path-uat-bridges.mjs','scripts/prepare-production-business-uat.mjs','scripts/prepare-production-business-uat-round7.mjs','scripts/prepare-production-business-uat-round8.mjs','scripts/prepare-production-business-uat-round9.mjs','scripts/prepare-production-business-uat-round10.mjs','scripts/verify-production-business-uat.mjs','tests/production-business-path-uat.spec.mjs','playwright.business-uat.config.mjs']){
...
console.log('Production business-path UAT verified: all nine end-to-end paths, stateful synthetic mutation protection, exact Admin/page readiness, visible Home Health intake approval, guarded Workforce navigation, pinned SPIRE app/deep-link generations with asynchronous census refresh and patient-click recovery, SCLS task continuity, payroll readiness, compliance continuity, and exact production synchronization are enforced.');

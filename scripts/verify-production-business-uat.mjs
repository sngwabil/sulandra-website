...
const chartReadyGeneration='20260810-spire-chart-ready-1';
...
const [installer,preparer,round7,round8,round9,round10,round12,payroll,test,config,workflow,pkg,interview,applicant,offer,workforceAdvanced,taskPage,navGuard,deepLink,chartReady,hhTokenBootstrap,spirePage,employeePortal]=await Promise.all([
  read('scripts/install-business-path-uat-bridges.mjs'),read('scripts/prepare-production-business-uat.mjs'),read('scripts/prepare-production-business-uat-round7.mjs'),read('scripts/prepare-production-business-uat-round8.mjs'),read('scripts/prepare-production-business-uat-round9.mjs'),read('scripts/prepare-production-business-uat-round10.mjs'),read('scripts/prepare-production-business-uat-round12.mjs'),read('assets/workforce-payroll-readiness.js'),read('tests/production-business-path-uat.spec.mjs'),read('playwright.business-uat.config.mjs'),read('.github/workflows/production-business-uat.yml'),read('package.json'),read('interview-admin-scheduler.js'),read('applicant-portal.html'),read('offer-acceptance.html'),read('api/src/workforce-advanced-routes.ts'),read('scls-tasks.html'),read('assets/employee-role-navigation-guard.js'),read('assets/spire-deep-link.js'),read('assets/spire-chart-ready.js'),read('assets/home-health-referral-token-bootstrap.js'),read('spire.html'),read('employee-portal.html')
]);

for(const marker of [contract,spireAppGeneration,deepLinkGeneration,'interview-admin-scheduler.js','applicant-portal.html','offer-acceptance.html','sclsTaskBoardLink','sclsTasksWorkflowLink','workforce-payroll-readiness.js','home-health-referral-token-bootstrap.js','companyComplianceLink','homeHealthReferralInboxLink','employeeHomeHealthReferralInboxLauncher','employee-role-navigation-guard.js','BUSINESS_UAT_NATIVE_DEEPLINK','BUSINESS_UAT_ASYNC_WORKSPACE_REFRESH','BUSINESS_UAT_CHART_FIRST','BUSINESS_UAT_CHART_STABILIZED','SpireOpenPatient'])expect(installer.includes(marker),`Business UAT installer missing ${marker}`);
...
expect(chartReady.includes(chartReadyGeneration)&&chartReady.includes('ensurePatientChart')&&chartReady.includes('forceChartActive')&&chartReady.includes('SpireOpenPatient')&&chartReady.includes('spireChartReady')&&chartReady.includes('spireChartPatientId')&&chartReady.includes('MutationObserver'),'SPIRE chart-readiness coordinator is incomplete');
expect(spirePage.includes(`spire-app-v2.js?v=${spireAppGeneration}`),'SPIRE page is not pinned to the asynchronous chart-stabilized application generation');
expect(spirePage.includes(`spire-chart-ready.js?v=${chartReadyGeneration}`),'SPIRE page is not pinned to the deterministic chart-readiness generation');
expect(spirePage.includes(`spire-deep-link.js?v=${deepLinkGeneration}`),'SPIRE page is not pinned to the coordinator-aware deep-link generation');
...
for(const rel of ['assets/workforce-payroll-readiness.js','assets/employee-role-navigation-guard.js','assets/spire-deep-link.js','assets/spire-chart-ready.js','assets/home-health-referral-token-bootstrap.js','scripts/install-business-path-uat-bridges.mjs','scripts/prepare-production-business-uat.mjs','scripts/prepare-production-business-uat-round7.mjs','scripts/prepare-production-business-uat-round8.mjs','scripts/prepare-production-business-uat-round9.mjs','scripts/prepare-production-business-uat-round10.mjs','scripts/prepare-production-business-uat-round12.mjs','scripts/verify-production-business-uat.mjs','tests/production-business-path-uat.spec.mjs','playwright.business-uat.config.mjs']){
...
console.log('Production business-path UAT verified: all nine end-to-end paths, stateful synthetic mutation protection, browser-error diagnostics, deterministic SPIRE chart-readiness coordination for Intake/eMAR/Incident, scoped Home Health intake approval, guarded Workforce navigation, asynchronous census refresh, SCLS task continuity, payroll readiness, compliance continuity, and exact production synchronization are enforced.');

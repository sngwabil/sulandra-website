...
const [installer,preparer,round7,round8,payroll,test,config,workflow,pkg,interview,applicant,offer,workforceAdvanced,taskPage,navGuard,deepLink,hhTokenBootstrap,spirePage]=await Promise.all([
  read('scripts/install-business-path-uat-bridges.mjs'),read('scripts/prepare-production-business-uat.mjs'),read('scripts/prepare-production-business-uat-round7.mjs'),read('scripts/prepare-production-business-uat-round8.mjs'),read('assets/workforce-payroll-readiness.js'),read('tests/production-business-path-uat.spec.mjs'),read('playwright.business-uat.config.mjs'),read('.github/workflows/production-business-uat.yml'),read('package.json'),read('interview-admin-scheduler.js'),read('applicant-portal.html'),read('offer-acceptance.html'),read('api/src/workforce-advanced-routes.ts'),read('scls-tasks.html'),read('assets/employee-role-navigation-guard.js'),read('assets/spire-deep-link.js'),read('assets/home-health-referral-token-bootstrap.js'),read('spire.html')
]);

for(const marker of [contract,'interview-admin-scheduler.js','applicant-portal.html','offer-acceptance.html','sclsTaskBoardLink','sclsTasksWorkflowLink','workforce-payroll-readiness.js','home-health-referral-token-bootstrap.js','companyComplianceLink','homeHealthReferralInboxLink','employeeHomeHealthReferralInboxLauncher','BUSINESS_UAT_NATIVE_DEEPLINK','BUSINESS_UAT_CHART_FIRST'])expect(installer.includes(marker),`Business UAT installer missing ${marker}`);
...
for(const rel of ['assets/workforce-payroll-readiness.js','assets/employee-role-navigation-guard.js','assets/spire-deep-link.js','assets/home-health-referral-token-bootstrap.js','scripts/install-business-path-uat-bridges.mjs','scripts/prepare-production-business-uat.mjs','scripts/prepare-production-business-uat-round7.mjs','scripts/prepare-production-business-uat-round8.mjs','scripts/verify-production-business-uat.mjs','tests/production-business-path-uat.spec.mjs','playwright.business-uat.config.mjs']){

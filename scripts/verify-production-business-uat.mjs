...
const [installer,preparer,round7,round8,round9,round10,round12,round13,payroll,test,config,workflow,pkg,interview,applicant,offer,workforceAdvanced,taskPage,navGuard,deepLink,chartReady,shellResilience,homeHealthRail,hhTokenBootstrap,spirePage]=await Promise.all([
  read('scripts/install-business-path-uat-bridges.mjs'),read('scripts/prepare-production-business-uat.mjs'),read('scripts/prepare-production-business-uat-round7.mjs'),read('scripts/prepare-production-business-uat-round8.mjs'),read('scripts/prepare-production-business-uat-round9.mjs'),read('scripts/prepare-production-business-uat-round10.mjs'),read('scripts/prepare-production-business-uat-round12.mjs'),read('scripts/prepare-production-business-uat-round13.mjs'),read('assets/workforce-payroll-readiness.js'),read('tests/production-business-path-uat.spec.mjs'),read('playwright.business-uat.config.mjs'),read('.github/workflows/production-business-uat.yml'),read('package.json'),read('interview-admin-scheduler.js'),read('applicant-portal.html'),read('offer-acceptance.html'),read('api/src/workforce-advanced-routes.ts'),read('scls-tasks.html'),read('assets/employee-role-navigation-guard.js'),read('assets/spire-deep-link.js'),read('assets/spire-chart-ready.js'),read('assets/spire-shell-resilience.js'),read('assets/home-health-rail-stability.js'),read('assets/home-health-referral-token-bootstrap.js'),read('spire.html')
]);

for(const marker of [contract,spireAppGeneration,chartReadyGeneration,deepLinkGeneration,'interview-admin-scheduler.js','applicant-portal.html','offer-acceptance.html','sclsTaskBoardLink','sclsTasksWorkflowLink','workforce-payroll-readiness.js','home-health-referral-token-bootstrap.js','home-health-rail-stability.js','companyComplianceLink','homeHealthReferralInboxLink','employeeHomeHealthReferralInboxLauncher','employee-role-navigation-guard.js','BUSINESS_UAT_NATIVE_DEEPLINK','BUSINESS_UAT_ASYNC_WORKSPACE_REFRESH','BUSINESS_UAT_CHART_FIRST','BUSINESS_UAT_CHART_STABILIZED','BUSINESS_UAT_CHART_WORKSPACE_STATE','BUSINESS_UAT_CANONICAL_SHELL_REPAIR','SpireEnsureShell','SpireOpenPatient'])expect(installer.includes(marker),`Business UAT installer missing ${marker}`);
...
expect(shellResilience.includes(shellResilienceGeneration)&&shellResilience.includes('CANONICAL_SINGLE_RUNTIME_REPAIR')&&shellResilience.includes('SpireEnsureShell')&&shellResilience.includes('NON_DESTRUCTIVE_SHELL_RECOVERY')&&shellResilience.includes('PRESERVE_ACTIVE_PATIENT_CHART'),'SPIRE single-runtime shell resilience contract is incomplete');
...
expect(spirePage.includes(`spire-app-v2.js?v=${spireAppGeneration}`)&&spirePage.includes(`spire-shell-resilience.js?v=${shellResilienceGeneration}`)&&spirePage.includes(`spire-chart-ready.js?v=${chartReadyGeneration}`)&&spirePage.includes(`spire-deep-link.js?v=${deepLinkGeneration}`),'SPIRE source page is missing the exact versioned app/shell/chart-ready/deep-link assets');
...
for(const marker of [contract,spireAppGeneration,shellResilienceGeneration,chartReadyGeneration,deepLinkGeneration,'Wait for exact production business-UAT deployment','playwright.business-uat.config.mjs','www.sulandrahealth.com','home-health-referral-token-bootstrap.js','home-health-rail-stability.js','companyComplianceLink','employeeHomeHealthReferralInboxLauncher','employee-role-navigation-guard.js','BUSINESS_UAT_NATIVE_DEEPLINK','BUSINESS_UAT_ASYNC_WORKSPACE_REFRESH','BUSINESS_UAT_CHART_FIRST','BUSINESS_UAT_CHART_STABILIZED','BUSINESS_UAT_CHART_WORKSPACE_STATE','BUSINESS_UAT_CANONICAL_SHELL_REPAIR','CANONICAL_SINGLE_RUNTIME_REPAIR','BUSINESS_UAT_IDEMPOTENT_CHART_ACTIVE','BUSINESS_UAT_DIRECT_CHART_OPEN','BUSINESS_UAT_PATIENT_CLICK_RECOVERY','SpireChartReady','ensurePatientChart','SpireEnsureShell','SpireOpenPatient','waitForAuthorizedPatient'])expect(workflow.includes(marker),`Business UAT workflow missing ${marker}`);
...
for(const rel of ['assets/workforce-payroll-readiness.js','assets/employee-role-navigation-guard.js','assets/spire-deep-link.js','assets/spire-chart-ready.js','assets/spire-shell-resilience.js','assets/home-health-rail-stability.js','assets/home-health-referral-token-bootstrap.js','scripts/install-business-path-uat-bridges.mjs','scripts/prepare-production-business-uat.mjs','scripts/prepare-production-business-uat-round7.mjs','scripts/prepare-production-business-uat-round8.mjs','scripts/prepare-production-business-uat-round9.mjs','scripts/prepare-production-business-uat-round10.mjs','scripts/prepare-production-business-uat-round12.mjs','scripts/prepare-production-business-uat-round13.mjs','scripts/verify-production-business-uat.mjs','tests/production-business-path-uat.spec.mjs','playwright.business-uat.config.mjs']){
...
console.log('Production business-path UAT verified: all nine end-to-end paths, stateful synthetic mutation protection, browser-error diagnostics, canonical single-runtime SPIRE shell recovery for Intake/eMAR/Incident, stable Home Health referral rail, exact Home Health intake-to-episode and POC review lifecycle, guarded Workforce navigation, asynchronous census refresh, SCLS task continuity, payroll readiness, compliance continuity, and exact production synchronization are enforced.');

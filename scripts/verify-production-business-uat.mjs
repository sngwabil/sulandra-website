...
const [installer,preparer,round7,round8,round9,payroll,test,config,workflow,pkg,interview,applicant,offer,workforceAdvanced,taskPage,navGuard,deepLink,hhTokenBootstrap,spirePage]=await Promise.all([
  read('scripts/install-business-path-uat-bridges.mjs'),read('scripts/prepare-production-business-uat.mjs'),read('scripts/prepare-production-business-uat-round7.mjs'),read('scripts/prepare-production-business-uat-round8.mjs'),read('scripts/prepare-production-business-uat-round9.mjs'),read('assets/workforce-payroll-readiness.js'),read('tests/production-business-path-uat.spec.mjs'),read('playwright.business-uat.config.mjs'),read('.github/workflows/production-business-uat.yml'),read('package.json'),read('interview-admin-scheduler.js'),read('applicant-portal.html'),read('offer-acceptance.html'),read('api/src/workforce-advanced-routes.ts'),read('scls-tasks.html'),read('assets/employee-role-navigation-guard.js'),read('assets/spire-deep-link.js'),read('assets/home-health-referral-token-bootstrap.js'),read('spire.html')
]);

for(const marker of [contract,spireAppGeneration,'interview-admin-scheduler.js','applicant-portal.html','offer-acceptance.html','sclsTaskBoardLink','sclsTasksWorkflowLink','workforce-payroll-readiness.js','home-health-referral-token-bootstrap.js','companyComplianceLink','homeHealthReferralInboxLink','employeeHomeHealthReferralInboxLauncher','BUSINESS_UAT_NATIVE_DEEPLINK','BUSINESS_UAT_CHART_FIRST','BUSINESS_UAT_CHART_STABILIZED','SpireOpenPatient'])expect(installer.includes(marker),`Business UAT installer missing ${marker}`);
...
expect(deepLink.includes('waitForAuthorizedPatient')&&deepLink.includes("query.get('patientId')")&&deepLink.includes("hash.get('tab')")&&deepLink.includes('fallbackOpenRequestedChart')&&deepLink.includes('directOpenRequestedChart')&&deepLink.includes('BUSINESS_UAT_DIRECT_CHART_OPEN')&&deepLink.includes('SpireOpenPatient'),'SPIRE patient/tab direct plus fallback deep-link bridge is incomplete');
expect(spirePage.includes(`spire-app-v2.js?v=${spireAppGeneration}`),'SPIRE page is not pinned to the chart-stabilized application generation');
...
for(const rel of ['assets/workforce-payroll-readiness.js','assets/employee-role-navigation-guard.js','assets/spire-deep-link.js','assets/home-health-referral-token-bootstrap.js','scripts/install-business-path-uat-bridges.mjs','scripts/prepare-production-business-uat.mjs','scripts/prepare-production-business-uat-round7.mjs','scripts/prepare-production-business-uat-round8.mjs','scripts/prepare-production-business-uat-round9.mjs','scripts/verify-production-business-uat.mjs','tests/production-business-path-uat.spec.mjs','playwright.business-uat.config.mjs']){
...
console.log('Production business-path UAT verified: all nine end-to-end paths, stateful synthetic mutation protection, exact Admin/page readiness, secure referral-token entry, pinned chart-stabilized SPIRE application plus direct/fallback deep links, direct Home Health Referral Inbox continuity, real SCLS task readiness, protected Workforce navigation, payroll readiness, compliance continuity, and exact production synchronization are enforced.');

...
const medicationPolicyUrl = '/assets/spire-medication-management-policy.js?v=20260816-top-manage-only-2';
const medicationLoaderRelative = 'assets/spire-medication-order-entry.js';
const medicationLoaderMarker = 'SPIRE_MEDICATION_ORDER_CANONICAL_LOADER_V3';
const medicationLoaderUrl = '/assets/spire-medication-order-entry.js?v=20260816-med-order-canonical-loader-3';
...
const medicationLoaderPath = path.join(root, medicationLoaderRelative);
const medicationLoader = await readFile(medicationLoaderPath, 'utf8');
if (!medicationLoader.includes(medicationLoaderMarker)) throw new Error(`SPIRE medication order loader is missing ${medicationLoaderMarker}`);
if (!medicationLoader.includes('spire-medication-order-entry-v2.js?v=20260816-med-order-v2-canonical-2')) throw new Error('SPIRE medication order loader is not pinned to the canonical V2 order workspace');
if (medicationLoader.includes('observe(document.documentElement')) throw new Error('SPIRE medication order loader must not observe the whole document');
new Function(medicationLoader);
const publishedMedicationLoaderPath = path.join(dist, medicationLoaderRelative);
await stat(publishedMedicationLoaderPath);
const publishedMedicationLoader = await readFile(publishedMedicationLoaderPath, 'utf8');
if (!publishedMedicationLoader.includes(medicationLoaderMarker)) throw new Error('Published SPIRE medication order loader is stale');
new Function(publishedMedicationLoader);
...
async function publishMaster(masterPath) {
  ...
  const loaderCount = (html.match(/\/assets\/spire-medication-order-entry\.js\?v=/g) || []).length;
  if (loaderCount !== 1) throw new Error(`SPIRE canonical medication loader must publish exactly once in ${path.relative(root, masterPath)}; found ${loaderCount}`);
  if (!(html.indexOf(medicationPolicyUrl) < html.indexOf(medicationLoaderUrl) && html.indexOf(medicationLoaderUrl) < html.indexOf(assetUrl))) {
    throw new Error('SPIRE medication policy and canonical order loader must load before adaptive chart tabs');
  }
...
for (const required of [medicationPolicyUrl, medicationLoaderUrl, assetUrl, 'data-view="flowsheets-view"', 'data-view="mar-view"', 'id="mainChartTabs"']) {
  if (!finalMaster.includes(required)) throw new Error(`Final SPIRE adaptive chart tab publication is missing ${required}`);
}

console.log(`SPIRE Orders UI published deterministically: ${medicationPolicyUrl} retires per-medication Manage controls, ${medicationLoaderUrl} owns the single styled Add Medication Order + Manage Orders toolbar, and ${assetUrl} loads afterward.`);

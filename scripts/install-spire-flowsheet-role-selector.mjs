...
// Older grid builds required this publication-time compatibility patch. The newer
// filed-history grid owns the same persistence concern in source and additionally
// reloads the selected calendar day and exposes every exact filed timestamp. Never
// try to rewrite the newer implementation back to the retired rolling-range code.
if (!grid.includes(filePersistenceMarker) && !grid.includes(filedHistoryMarker)) {
...
if (grid.includes(filedHistoryMarker)) {
  for (const marker of [
    filedHistoryMarker,
    'function queryWindow()',
    'function mergeFiledEntryColumns()',
    'runtime.selectedDate',
    'mergeFiledEntryColumns();',
    'filed observation',
  ]) {
    if (!grid.includes(marker)) throw new Error(`SPIRE filed-history grid missing ${marker}`);
  }
} else {
...
catch (error) { throw new Error(`SPIRE master flowsheet grid syntax error after persistence verification: ${error instanceof Error ? error.message : String(error)}`); }
...
if (!master.includes(gridUrl)) throw new Error('SPIRE master did not cache-bust the filed-history grid');

await writeFile(masterPath, master, 'utf8');

const published = await readFile(masterPath, 'utf8');
if (!published.includes(gridUrl)) throw new Error('SPIRE master publication lost the filed-history grid');
...
console.log('SPIRE flowsheet publication preserves confirmed filed values across File and full reloads, reloads the selected date, surfaces exact filing timestamps, retains Nurse-specific task navigation with viewport scrolling, and applies the final clinical attribution/MAR/Notes regression guard.');

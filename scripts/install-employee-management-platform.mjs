...
for (const importLine of [permissionsImport, employeeImport, selfServiceImport, complianceImport, collaborationImport]) {
  if (!bootstrap.includes(importLine)) bootstrap = bootstrap.replace(careersImport, `${careersImport}\n${importLine}`);
}
for (const registerLine of [permissionsRegister, employeeRegister, selfServiceRegister, complianceRegister, collaborationRegister]) {
...
console.log('Employee 360 permissions, management, self-service, compliance, collaboration, approvals, feedback, recognition, and notifications are installed.');

...
for (const importLine of [permissionsImport, employeeImport, selfServiceImport]) {
  if (!bootstrap.includes(importLine)) bootstrap = bootstrap.replace(careersImport, `${careersImport}\n${importLine}`);
}
for (const registerLine of [permissionsRegister, employeeRegister, selfServiceRegister]) {
...
console.log('Employee 360 scoped permissions, management routes, and approved self-service records are installed.');

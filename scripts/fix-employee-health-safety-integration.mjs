...
for(const name of priorVerifiers){const target=path.join(root,'scripts',name);let source=await readFile(target,'utf8');if(source.includes(healthRegister))continue;if(!source.includes(careersRegister))throw new Error(`Unable to locate Careers registration in ${name}`);source=source.replace(careersRegister,`${healthRegister}\n\n${careersRegister}`);await writeFile(target,source,'utf8')}

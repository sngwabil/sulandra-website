...
must(payroll.includes("code:'PAYROLL_PERIOD_NOT_READY'")||payroll.includes("code: 'PAYROLL_PERIOD_NOT_READY'"),'fail-closed readiness blocker is missing');
must(payroll.includes("code:'PAYROLL_FINGERPRINT_MISMATCH'")||payroll.includes("code: 'PAYROLL_FINGERPRINT_MISMATCH'"),'stale payroll evidence blocker is missing');
must(payroll.includes("action:'REOPEN'")||payroll.includes("action: 'REOPEN'"),'audited controlled reopen is missing');
must(payroll.includes("action:'EXPORT'")||payroll.includes("action: 'EXPORT'"),'payroll export decision audit is missing');
must(payroll.includes('directPayrollProviderSubmission:false')||payroll.includes('directPayrollProviderSubmission: false'),'export boundary must not imply direct payroll-provider submission');
must(payroll.includes('const payrollAuthority ='),'payroll authority policy is missing');
for(const role of ['ADMINISTRATOR','HR_MANAGER','CEO','DOO'])must(payroll.includes(`'${role}'`),`${role} is missing from the payroll authority policy`);
...
for(const name of lifecycle){const script=String(pkg.scripts?.[name]||'');must(script.includes('install-payroll-time-attendance-lockouts.mjs'),`${name} does not run payroll lock installer`);must(script.includes('verify-payroll-time-attendance-lockouts.mjs'),`${name} does not run payroll lock verifier`);}

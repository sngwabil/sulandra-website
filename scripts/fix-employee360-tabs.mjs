...
const operationalController = source.includes('function switchTab(tab)')
  && source.includes("$('tabs').addEventListener('click'")
  && source.includes("document.querySelectorAll('[data-open-tab]')");
...
} else if (operationalController) {
  console.log('Employee 360 operational tab controller verified; legacy handler rewrite is not required.');
} else {

...
await verifyOptionalAsset(path.join('assets','it-agent-chatgpt-composer-layout.css'),['grid-column:1/4','grid-row:2','#agentArtifacts:not(:has(.artifact-row))','grid-row:3'],'ChatGPT composer CSS');
...
await verifyOptionalAsset(path.join('assets','it-agent-action-center-tab.js'),['IT_AGENT_ACTION_CENTER_TAB_V2','data-itws-view','Action Center','itwsActionCenterView','itws-action-center-panel','data-itws-view="overview"'],'Action Center tab JS');
...
for(const relative of ['it-solutions.html',path.join('dist-web','it-solutions.html')]){
  ...
  need(html,'/assets/it-agent-chatgpt-composer-layout.css?v=20260829-composer-1',relative);
  ...
  need(html,'/assets/it-agent-action-center-tab.js?v=20260829-action-tab-2',relative);
}
if(failures.length){console.error('IT Agent workspace verification failed:\n- '+failures.join('\n- '));process.exit(1)}
console.log('IT Agent chat-first workspace verified: owner-scoped conversation history, Action Center under Operations, ChatGPT-style full-width prompt, and grounded live execution activity are wired without widening action permissions.');

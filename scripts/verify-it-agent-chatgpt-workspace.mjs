...
for(const relative of ['it-solutions.html',path.join('dist-web','it-solutions.html')]){
  ...
  need(html,'/assets/it-agent-action-center-tab.js?v=20260829-action-tab-1',relative);
}
if(failures.length){console.error('IT Agent workspace verification failed:\n- '+failures.join('\n- '));process.exit(1)}
console.log('IT Agent chat-first workspace verified: owner-scoped conversation history, Action Center tab, ChatGPT-style full-width prompt with separate attachment/control rows, and grounded live execution activity are wired without widening action permissions.');

...
for (const marker of [
  '/public/client-service-requests','/api/admin/client-service-requests','/start-intake','route-to-requested-company',
  'UPDATE_CLIENT_SERVICE_REQUEST','START_CLIENT_INTAKE','ROUTE_PRELAUNCH_INTEREST_TO_APPROVED_PROVIDER',
  'ClientServiceRequestRoutingEvent','requestedLegalEntityId','intakeMode','sourcePath','OPERATIONAL','PRELAUNCH_INTEREST',
  'ENTERPRISE_CONSULTATION','formalProviderIntakeEnabled','requireEntityManageAccess','requireEnterpriseOwner','SpireIntakeImport','"legalEntityId"=$2',
]) expect(route.includes(marker), `Missing company client-intake backend marker: ${marker}`);
for (const marker of [
  'ClientServiceRequest_requested_entity_fkey','ClientServiceRequest_intake_mode_check','ClientServiceRequest_routing_shape_check',
  'ClientServiceRequestRoutingEvent','SpireIntakeImport_entity_status_created_idx','ALTER COLUMN "legalEntityId" SET NOT NULL',
...
for (const marker of [
  'entityAccessOf(res)','requireEntityManageAccess(access)','"SpireIntakeImport" ("id","organizationId","legalEntityId"',
  'WHERE "organizationId"=$1 AND "legalEntityId"=$2 AND "id"=$3','"SpireClinicalAuditEvent" WHERE "organizationId"=$1 AND "legalEntityId"=$2',
]) expect(clinical.includes(marker), `Missing entity-scoped SPIRE intake control: ${marker}`);
for (const marker of [
  'SulandraCompanyContext?.headers?.()','SulandraCompanyContext?.initialize?.()','sulandra:company-change','PRELAUNCH_INTEREST',
  'ENTERPRISE_CONSULTATION','requestedCompanyReadinessReason','formalIntakeAvailable','route-to-requested-company','cannot be treated as accepted provider service',
...
for (const source of [publicFormSource,publicBridgeSource]) {
  for (const marker of ['companyCode','sourcePath','PRELAUNCH_INTEREST']) expect(source.includes(marker), `Public intake submission is missing routing marker: ${marker}`);
}

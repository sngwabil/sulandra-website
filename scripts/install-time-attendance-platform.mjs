...
if (!bootstrap.includes(attendanceImport)) bootstrap = bootstrap.replace(geofenceImport, `${geofenceImport}\n${attendanceImport}`);
if (!bootstrap.includes(careersRegister)) throw new Error('Unable to locate careers route registration anchor');
for (const line of [geofenceRegister, attendanceRegister]) {
...
try {
  ...
  html = html.replace(/\s*<script src="\/assets\/time-attendance-geofence\.js(?:\?v=[^"']+)?"><\/script>\s*/g, '\n');
  html = html.replace('</body>', '  <script src="/assets/time-attendance-geofence.js?v=20260805-gps-1"></script>\n</body>');
  await writeFile(attendancePage, html, 'utf8');
} catch (error) {
  if (error?.code !== 'ENOENT') throw error;
}

console.log('Time and Attendance routes, GPS geofencing, manual punch review, and static portal navigation are installed.');

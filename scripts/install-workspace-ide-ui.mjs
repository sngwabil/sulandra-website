...
for(const marker of ['SULANDRA_CODEBASE_V1','release/sulandra-1.0','itwsSulandraCodebaseButton','requestFullscreen','openCodebaseFromGesture','SulandraDockableWorkspace','MAX_FILE_BYTES','blockedPath','/api/it-solutions/codebase/tree','/api/it-solutions/codebase/file','credentials:\'same-origin\''])if(!codebaseJs.includes(marker))throw new Error(`Sulandra Codebase JavaScript missing ${marker}`);
if(/api\.github\.com|\/git\/trees\/|\/git\/blobs\//.test(codebaseJs))throw new Error('Sulandra Codebase browser runtime must use the authenticated same-origin source API instead of direct GitHub API calls');

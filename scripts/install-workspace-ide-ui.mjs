...
for(const marker of ['SULANDRA_CODEBASE_V1','release/sulandra-1.0','itwsSulandraCodebaseButton','requestFullscreen','openCodebaseFromGesture','SulandraDockableWorkspace','MAX_FILE_BYTES','blockedPath','/api/it-solutions/codebase/tree','/api/it-solutions/codebase/file'])if(!codebaseJs.includes(marker))throw new Error(`Sulandra Codebase JavaScript missing ${marker}`);
for(const marker of ['SULANDRA_CODEBASE_API_BRIDGE_V1','sulandra-website-production-5fc4.up.railway.app','/api/it-solutions/codebase/','Authorization','Bearer ${bearer}','SulandraCompanyContext','credentials:\'omit\''])if(!codebaseApiBridge.includes(marker))throw new Error(`Sulandra Codebase API bridge missing ${marker}`);

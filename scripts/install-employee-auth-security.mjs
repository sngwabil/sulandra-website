...
// These edits must be safe when typecheck and build run sequentially in the same
// checkout. Older logic re-added mfaCode and sessionId on every invocation.
...
if(source.includes("    res.json(buildSessionPayload(account));")){
...
console.log('Employee authentication installer is idempotent and uses matching JWT/server session IDs, revocable sessions, portal controls, MFA, login history, scope enforcement and auth administration routes.');

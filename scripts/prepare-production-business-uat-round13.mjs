...
replaceExact(
  ...
  "const intakesRail=page.locator('[data-rail=\"intakes\"]');if(await intakesRail.isVisible().catch(()=>false))await intakesRail.click();await clickVisible(page,'[data-open-episode]');await expect.poll(()=>state.episode).toBe(true);",
  'Home Health visible Referrals transition before Open Episode',
);

await writeFile(target,source,'utf8');
console.log('Applied round-thirteen business UAT corrections: exact Home Health intake-to-episode route, episode readiness/detail shape, real Plan of Care DRAFT → REVIEW → ACTIVE lifecycle, and visible Referrals transition before Open Episode.');

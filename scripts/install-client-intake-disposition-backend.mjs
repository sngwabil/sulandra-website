...
const correctionMarker = 'CLIENT_INTAKE_APPROVED_CORRECTION_V2';
...
if (!source.includes(correctionMarker)) {
  const anchor = "  app.get('/api/admin/client-intakes/:caseId/duplicate-candidates'";
  if (!source.includes(anchor)) throw new Error('Client Intake approved-correction route anchor was not found');
  const route = `  /* ${correctionMarker}: approved packets remain immutable until an authorized reviewer explicitly opens a documented correction cycle. */\n` +
`  app.post('/api/admin/client-intakes/:caseId/reopen-correction',async(req,res,next)=>{try{const a=authOf(res);ensureReview(a);const reason=clean(req.body?.reason,6000);if(!reason)throw httpError(400,'Enter the reason this approved intake must be corrected');const caseRow=await requireCase(prisma,a,req.params.caseId),status=String(caseRow.status||'');if(status!=='APPROVED')throw httpError(409,'Only an approved intake can be reopened for correction');const openedAt=new Date().toISOString(),metadata={correctionCycle:'OPEN',correctionReason:reason,correctionOpenedAt:openedAt,correctionOpenedById:a.userId,previousStatus:'APPROVED',patientId:caseRow.patientId??null};await prisma.$executeRawUnsafe(\`UPDATE "ClientIntakeCase" SET "status"='REVIEW_REQUIRED',"reviewNotes"=$1,"reviewedAt"=NULL,"reviewedById"=NULL,"approvedAt"=NULL,"approvedById"=NULL,"metadata"=COALESCE("metadata",'{}'::jsonb)||$2::jsonb,"updatedAt"=NOW() WHERE "organizationId"=$3 AND "legalEntityId"=$4 AND "id"=$5 AND "status"='APPROVED'\`,reason,JSON.stringify(metadata),a.organizationId,selectedEntity(a),req.params.caseId);await event(prisma,a,req.params.caseId,'INTAKE_REOPENED_FOR_CORRECTION',{reason,patientId:caseRow.patientId??null,openedAt});await audit?.(a,'REOPEN_APPROVED_CLIENT_INTAKE_FOR_CORRECTION','ClientIntakeCase',req.params.caseId,{legalEntityId:selectedEntity(a),reason,patientId:caseRow.patientId??null,openedAt});res.json({data:{status:'REVIEW_REQUIRED',patientId:caseRow.patientId??null,reason}});}catch(e){next(e);}});\n\n`;
  source = source.replace(anchor, route + anchor);
}

await writeFile(target, source, 'utf8');
console.log('Client Intake disposition backend installed: draft rejection, retained archive lifecycle, and audited approved-intake correction cycles are available.');

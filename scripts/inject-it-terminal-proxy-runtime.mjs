...
app.get('/api/it-solutions/terminal/sessions/:sessionId/output',terminalAdminRoles,async(req,res,next)=>{try{const sessionId=terminalIdentifierSchema.parse(req.params.sessionId);const cursor=Math.max(0,Math.trunc(Number(req.query.cursor)||0));const data=await terminalWorkerRequest(res,'/sessions/'+encodeURIComponent(sessionId)+'/output?cursor='+encodeURIComponent(String(cursor)),{timeoutMs:10000});res.json({data})}catch(error){next(error)}});

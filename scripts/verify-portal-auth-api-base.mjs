...
const production='https://sulandra-website-production-5fc4.up.railway.app';
const canonical=String(process.env.VITE_API_URL||production).trim().replace(/\/$/,'');
...
for(const relative of files){
  try{
    ...
    if(apiRequired.has(relative) && !source.includes(canonical))failures.push(`${relative} does not reference the configured Railway API hostname`);
    ...
    if(relative==='admin-railway.js' && /response\.status\s*===\s*401\)\s*signOut\(/.test(source)) failures.push('admin-railway.js still clears the global login when one protected feature endpoint returns 401');
...
console.log(`Portal authentication verified: login/Admin use ${canonical}, Employee Portal uses the sign-in-once session cache, and module-level authorization failures do not destroy the global login.`);
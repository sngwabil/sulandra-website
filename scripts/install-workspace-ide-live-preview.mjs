...
const copyWorkspaceResponseHeaders = (upstream, res, { embeddable = false } = {}) => {
  for (const [key, value] of upstream.headers.entries()) {
    ...
    if (workspaceHopHeaders.has(lower) || lower === 'content-length' || lower === 'x-frame-options' || lower === 'content-security-policy') continue;
...
if (source.includes("const docker = new Docker")) {
  // Execution plane: resource baseline plus a fixed internal IDE port proxy.
  ...
  const executorProxy = `${proxyHeaderHelpers}
...
`;
  ...
  // Railway gateway: browser JWT -> short-lived session ticket -> executor proxy.
  ...
  const gatewayProxy = `${proxyHeaderHelpers}
...
const validateWorkspaceProxyPath = rest => {
  ...
  return Number.isInteger(port) && port >= 1024 && port <= 65535 && ![9000, 13337].includes(port);
};
const proxyBrowserWorkspace = async (req, res, next) => {
  try {
    ...
    if (!['GET','HEAD'].includes(String(req.method || 'GET').toUpperCase())) { init.body = req; init.duplex = 'half'; }
    const upstream = await fetch(executionUrl(`/v1/ide/${encodeURIComponent(sessionId)}${rest}${parsed.search}`), init);
    res.statusCode = upstream.status;
...
app.post('/workspace/ticket', async (req, res) => {
  ...
  const port = req.body?.port === undefined || req.body?.port === null || req.body?.port === '' ? null : Number(req.body.port);
  if (!/^[A-Za-z0-9_-]+$/.test(sessionId)) return res.status(400).json({ error: 'Valid terminal sessionId is required' });
  if (port !== null && (!Number.isInteger(port) || port < 1024 || port > 65535 || [9000, 13337].includes(port))) return res.status(400).json({ error: 'Preview port must be 1024-65535 and not a reserved terminal port' });
  ...
  const path = port === null ? `${base}/` : `${base}/proxy/${port}/`;
  res.set('Cache-Control', 'no-store');
  res.json({ ok: true, sessionId, port, expiresIn: workspaceTicketSeconds, url: `${path}?ticket=${encodeURIComponent(ticket)}` });
});
`;

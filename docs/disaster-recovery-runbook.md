...
This runbook defines the disaster-recovery engineering controls for the current Sulandra production line, including PostgreSQL backup, restore, validation, incident recovery, and evidence retention. It is designed to prevent an untested backup from being treated as a recovery capability.

The canonical production branch is `release/sulandra-1.0`. This document does not authorize a production restore by itself.

## Recovery objectives

The following are engineering targets and must be formally approved before they are treated as production commitments:
...
Sulandra production is not disaster-recovery ready until all of the following are evidenced:
...
The repository provides `scripts/dr-create-postgres-backup.sh` as a portable logical-backup control. It creates a PostgreSQL custom-format dump with:
...
- Employee login reaches the correct employee authentication flow.
- Admin login reaches the separate administrator authentication flow.
- Admin workspace loads with the expected company context.
- SIA loads through the authenticated employee environment.

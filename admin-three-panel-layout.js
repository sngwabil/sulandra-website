(function(){
  ...
  async function loadLiveActivity(){
    ...
    const results=await Promise.allSettled([api('/api/admin/employees'),api('/api/admin/applications?limit=40'),api('/api/admin/interview-slots')]);
    const employeePayload=results[0].status==='fulfilled'?results[0].value:[];const employeeRows=Array.isArray(employeePayload)?employeePayload:[];const apps=results[1].status==='fulfilled'?(Array.isArray(results[1].value)?results[1].value:[]):[];const slotsPayload=results[2].status==='fulfilled'?(results[2].value||{}):{};const slots=Array.isArray(slotsPayload)?slotsPayload:(slotsPayload.slots||[]);
    ...
    const clocked='—';const employees=employeeRows.length;

...
for(const [key,p] of Object.entries(PERSONAS))test(`${p.label}: login-first production UAT`,async({page})=>{
  ...
  else if(key==='hr'){await absent(page,'#employeeMyShiftLauncher','#employeeLiveSpireLauncher');await expect(page.locator('#employeeCompanyDocumentsLauncher')).toBeVisible();await open(page,'#employeeHr360Launcher','/employee360.html','Employee 360 Enterprise Workspace');}

...
test('SPIRE selected home opens user master-template chart without render loops',async({page})=>{
  ...
  await page.route(`${API}/**`,async route=>{
    const request=route.request();const url=new URL(request.url());const path=url.pathname;const method=request.method().toUpperCase();
  ...
  const strip=page.locator('#spirePatientStrip');const chart=page.locator('#spireChartWorkspace');
  ...
  const workstation=await page.evaluate(()=>{
    const chart=document.getElementById('spireChartWorkspace');const tabs=chart?.querySelector(':scope > .chart-tabs');
    const inactiveTab=chart?.querySelector('[data-chart-tab="results-review"]');const activeTab=chart?.querySelector('[data-chart-tab="chart-review"]');const flowsheetsTab=chart?.querySelector('[data-spmt-special="flowsheets"]');
    const left=document.querySelector('.spire-left-rail');const right=document.querySelector('.spire-right-rail');const strip=document.getElementById('spirePatientStrip');const shell=document.querySelector('.spire-shell');
    const brand=document.querySelector('.spire-brand');const brandText=document.querySelector('.spire-brand strong');const logoMark=document.querySelector('.spire-logo-mark');const topbar=document.querySelector('.spire-topbar');const topAction=document.querySelector('.spire-top-actions button');const masterToolbar=document.getElementById('spireMasterToolbar');
    const stylesheets=[...document.styleSheets].map(sheet=>sheet.href||'');
    const stylesheet=stylesheets.find(href=>href.includes('spire-clinical-workstation.css'))||'';const controlStylesheet=stylesheets.find(href=>href.includes('spire-sulandra-controls-final.css'))||'';const masterStylesheet=stylesheets.find(href=>href.includes('spire-user-template-integration.css'))||'';const masterLayoutStylesheet=stylesheets.find(href=>href.includes('spire-user-template-layout-fix.css'))||'';
    const chartRect=chart?.getBoundingClientRect();const tabsRect=tabs?.getBoundingClientRect();const stripRect=strip?.getBoundingClientRect();const shellRect=shell?.getBoundingClientRect();
    const inactiveStyle=inactiveTab?getComputedStyle(inactiveTab):null;const activeStyle=activeTab?getComputedStyle(activeTab):null;const inactiveBefore=inactiveTab?getComputedStyle(inactiveTab,'::before'):null;const topbarStyle=topbar?getComputedStyle(topbar):null;const topActionStyle=topAction?getComputedStyle(topAction):null;const toolbarStyle=masterToolbar?getComputedStyle(masterToolbar):null;const brandStyle=brand?getComputedStyle(brand):null;const brandTextStyle=brandText?getComputedStyle(brandText):null;
    return {stylesheet,controlStylesheet,masterStylesheet,masterLayoutStylesheet,masterRuntime:window.SpireMasterTemplate?.version||'',appFontFamily:getComputedStyle(document.querySelector('.spire-app')).fontFamily,chartDisplay:chart?getComputedStyle(chart).display:'',tabsDirection:tabs?getComputedStyle(tabs).flexDirection:'',tabBeforeDisplay:inactiveBefore?.display||'',tabBorderRadius:inactiveStyle?.borderRadius||'',tabFontWeight:inactiveStyle?.fontWeight||'',inactiveTabBackground:inactiveStyle?.backgroundColor||'',activeTabBackground:activeStyle?.backgroundColor||'',flowsheetsTabText:flowsheetsTab?.textContent?.trim()||'',topbarBackgroundImage:topbarStyle?.backgroundImage||'',topbarHeight:topbar?.getBoundingClientRect().height||0,topbarBorderBottomColor:topbarStyle?.borderBottomColor||'',topbarBorderBottomWidth:topbarStyle?.borderBottomWidth||'',topActionBackground:topActionStyle?.backgroundColor||'',topActionColor:topActionStyle?.color||'',topActionBorderRadius:topActionStyle?.borderRadius||'',masterToolbarBackground:toolbarStyle?.backgroundColor||'',masterToolbarHeight:masterToolbar?.getBoundingClientRect().height||0,leftDisplay:left?getComputedStyle(left).display:'',rightDisplay:right?getComputedStyle(right).display:'',rightWidth:right?right.getBoundingClientRect().width:0,patientStripWidth:stripRect?.width||0,patientStripHeight:stripRect?.height||0,patientStripRight:stripRect?.right||0,shellLeft:shellRect?.left||0,chartTop:chartRect?.top||0,tabsTop:tabsRect?.top||0,tabsWidth:tabsRect?.width||0,tabsHeight:tabsRect?.height||0,brandWidth:brand?.getBoundingClientRect().width||0,brandBackground:brandStyle?.backgroundColor||'',brandTextBackground:brandTextStyle?.backgroundColor||'',brandTextColor:brandTextStyle?.color||'',brandFontStyle:brandTextStyle?.fontStyle||'',brandFontWeight:brandTextStyle?.fontWeight||'',brandText:brandText?.textContent?.trim()||'',logoMarkDisplay:logoMark?getComputedStyle(logoMark).display:''};
  });
  ...
  expect(workstation.masterStylesheet).toContain('spire-user-template-integration.css?v=20260812-user-master-template-4');
  expect(workstation.masterLayoutStylesheet).toContain('spire-user-template-layout-fix.css?v=20260812-user-master-template-4');
  expect(workstation.masterRuntime).toBe('20260812-user-master-template-2');
  expect(workstation.appFontFamily).toContain('Segoe UI');
  expect(workstation.chartDisplay).toBe('grid');expect(workstation.tabsDirection).toBe('row');expect(workstation.tabBeforeDisplay).toBe('none');expect(workstation.tabBorderRadius).toBe('0px');expect(Number(workstation.tabFontWeight)).toBeLessThanOrEqual(600);
  expect(['rgba(0, 0, 0, 0)','rgb(216, 228, 240)','rgb(226, 232, 240)']).toContain(workstation.inactiveTabBackground);expect(workstation.activeTabBackground).toBe('rgb(255, 255, 255)');expect(workstation.flowsheetsTabText).toBe('Flowsheets');
  expect(workstation.topbarBackgroundImage).toContain('linear-gradient');expect(workstation.topbarHeight).toBeGreaterThanOrEqual(39);expect(workstation.topbarHeight).toBeLessThanOrEqual(41);expect(workstation.topbarBorderBottomColor).toBe('rgb(30, 41, 59)');expect(workstation.topbarBorderBottomWidth).toBe('1px');
  expect(workstation.topActionBackground).toMatch(/rgba?\(/);expect(workstation.topActionColor).toBe('rgb(255, 255, 255)');expect(workstation.topActionBorderRadius).toBe('3px');
  expect(workstation.masterToolbarBackground).toBe('rgb(153, 0, 0)');expect(workstation.masterToolbarHeight).toBeGreaterThanOrEqual(35);expect(workstation.masterToolbarHeight).toBeLessThanOrEqual(37);
  expect(workstation.leftDisplay).toBe('none');expect(workstation.rightDisplay).not.toBe('none');expect(workstation.rightWidth).toBeGreaterThanOrEqual(270);expect(workstation.rightWidth).toBeLessThanOrEqual(290);expect(workstation.patientStripWidth).toBeGreaterThanOrEqual(270);expect(workstation.patientStripWidth).toBeLessThanOrEqual(290);expect(workstation.patientStripHeight).toBeGreaterThan(500);expect(Math.abs(workstation.patientStripRight-workstation.shellLeft)).toBeLessThan(3);expect(workstation.tabsWidth).toBeGreaterThan(workstation.tabsHeight*6);expect(Math.abs(workstation.chartTop-workstation.tabsTop)).toBeLessThan(3);
  ...
  await page.locator('[data-chart-tab="chart-review"]').click();await expect(page.locator('#spireChartTabBody .spire-cr-layout')).toBeVisible();const chartReviewCallsAfterNavigation=chartReviewCalls;
  await page.waitForTimeout(1800);await expect(chart).toHaveClass(/active/);await expect(page.locator('#spireMasterToolbar')).toBeVisible();await expect(page.locator('#spireChartTabBody .spire-cr-layout')).toHaveCount(1);expect(chartReviewCalls,'Chart Review kept refetching after the explicit navigation settled').toBe(chartReviewCallsAfterNavigation);

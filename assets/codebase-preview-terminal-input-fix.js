 ...
 * - Preview remains a dark glossy screen until the user explicitly presses Open.
 * - Opening/reattaching a terminal never navigates Preview by itself.
 * - CodeMirror and xterm receive durable pointer focus after workspace rerenders.
 * - Focus fallbacks keep hardware/onscreen keyboard input usable without globally
 *   swallowing keyboard events from the rest of Codebase.
 */
(()=>{
  'use strict';
  if(window.__SULANDRA_CODEBASE_PREVIEW_TERMINAL_INPUT_V2__)return;
  ...
  const styleText=`
    #view-preview{
      ...
      background:radial-gradient(circle at 18% 0%,rgba(46,204,113,.08),transparent 34%),radial-gradient(circle at 88% 12%,rgba(66,165,245,.08),transparent 30%),linear-gradient(180deg,#020407 0%,#000 100%)!important;
      isolation:isolate;
    }
    #view-preview>#codebase-preview-toolbar{
      position:absolute!important;z-index:6!important;top:8px!important;left:8px!important;right:8px!important;
      height:34px!important;min-height:34px!important;display:flex!important;align-items:center!important;gap:7px!important;
      margin:0!important;padding:0 7px!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:8px!important;
    ...
    #codebase-preview-toolbar .codebase-preview-open:hover,#codebase-preview-toolbar .codebase-preview-actions button:hover{border-color:rgba(79,195,247,.32)!important;background:rgba(79,195,247,.07)!important;color:#fff!important}
    ...
    .terminal-view{cursor:text!important;outline:none!important}.terminal-view .xterm,.terminal-view .xterm-screen{height:100%!important}.terminal-view:focus-within .xterm-cursor-layer,.terminal-view[data-codebase-focused="true"] .xterm-cursor-layer{opacity:1!important}
    .CodeMirror[data-codebase-input-bound="1"]{outline:none!important}
  `;

  const installStyle=()=>{
    let style=document.getElementById(styleId);
    if(style)return style;
    style=document.createElement('style');style.id=styleId;style.textContent=styleText;document.head.appendChild(style);return style;
  };
  ...
  const setPreviewDark=(message='')=>{
    const iframe=document.getElementById('railway-preview-iframe');
    if(!iframe)return;
    try{iframe.removeAttribute('src');iframe.srcdoc=darkDoc;iframe.dataset.codebasePreviewState='idle'}catch{}
    ...
    const focusNow=()=>{
      ...
      requestAnimationFrame(()=>{
        ...
        requestAnimationFrame(()=>{
          ...
          if(!inside){try{container.focus({preventScroll:true})}catch{}}
        });
      });
    };
    if(container.dataset.codebaseKeyboardBound!=='1'){
      container.dataset.codebaseKeyboardBound='1';
      ...
      container.addEventListener('focusout',()=>{container.dataset.codebaseFocused='false'});
      container.addEventListener('keydown',event=>{
        if(event.target!==container)return;
        const data=terminalDataFromKey(event);if(!data)return;
  ...
  const editorFromWrapper=wrapper=>{
    for(const cm of Object.values(editors())){
      try{if(cm?.getWrapperElement?.()===wrapper)return cm}catch{}
    }
    return null;
  };
  const editorFallbackKey=(cm,event)=>{
    ...
    const commands={Enter:null,Backspace:'delCharBefore',Delete:'delCharAfter',ArrowLeft:'goCharLeft',ArrowRight:'goCharRight',ArrowUp:'goLineUp',ArrowDown:'goLineDown',Home:'goLineStartSmart',End:'goLineEnd',PageUp:'goPageUp',PageDown:'goPageDown'};
    if(event.key==='Enter'){cm.replaceSelection('\n','end','+input');return true}
    if(event.key==='Tab'){cm.replaceSelection('\t','end','+input');return true}
  ...
  const bindEditor=(cm,{focus=false}={})=>{
    ...
    const wrapper=cm.getWrapperElement();if(!wrapper)return;
    wrapper.tabIndex=0;wrapper.dataset.codebaseInputBound='1';
    const focusNow=()=>{
      try{cm.focus()}catch{}
      requestAnimationFrame(()=>{
        const active=document.activeElement;
        if(active&&(active===wrapper||wrapper.contains(active)))return;
        try{wrapper.focus({preventScroll:true})}catch{try{wrapper.focus()}catch{}}
      });
    };
    if(wrapper.dataset.codebaseKeyboardBound!=='1'){
      wrapper.dataset.codebaseKeyboardBound='1';
      wrapper.addEventListener('pointerdown',()=>requestAnimationFrame(focusNow),true);
      wrapper.addEventListener('touchstart',()=>requestAnimationFrame(focusNow),{capture:true,passive:true});
      wrapper.addEventListener('click',()=>requestAnimationFrame(focusNow),true);
      wrapper.addEventListener('keydown',event=>{
        if(event.target!==wrapper)return;
        if(!editorFallbackKey(cm,event))return;
        event.preventDefault();event.stopPropagation();
      });
    }
    if(focus)requestAnimationFrame(focusNow);
  ...
  const bindInputs=()=>{
    ...
    for(const [id,term] of Object.entries(terminals())){
      ...
      if(container)bindTerminal(term,container,id,{focus:tabs()[0]?.id===id&&document.activeElement===document.body});
    }
    for(const cm of Object.values(editors()))bindEditor(cm);
    document.querySelectorAll('textarea[id^="editor-"]:not([style*="display: none"])').forEach(textarea=>{
      if(textarea.dataset.codebaseNativeFocusBound==='1')return;
      textarea.dataset.codebaseNativeFocusBound='1';textarea.addEventListener('pointerdown',()=>textarea.focus(),true);textarea.addEventListener('click',()=>textarea.focus(),true);
    });
  };
  const scheduleBindings=()=>{clearTimeout(bindingTimer);bindingTimer=setTimeout(bindInputs,90);setTimeout(bindInputs,180)};

  const previousInitXterm=window.initXterm;
  if(typeof previousInitXterm==='function'&&previousInitXterm.__codebaseInputV2!==true){
    const wrapped=function(containerId,tabId){
      const result=previousInitXterm(containerId,tabId);
      requestAnimationFrame(()=>requestAnimationFrame(()=>{
        const container=document.getElementById(containerId);const term=terminals()[tabId];
        if(container&&term)bindTerminal(term,container,tabId,{focus:tabs()[0]?.id===tabId});
      }));
      return result;
    };
    wrapped.__codebaseInputV2=true;window.initXterm=wrapped;
  }

  const previousRenderWorkspace=window.renderWorkspace;
  if(typeof previousRenderWorkspace==='function'&&previousRenderWorkspace.__codebaseInputV2!==true){
    const wrapped=function(...args){const result=previousRenderWorkspace.apply(this,args);scheduleBindings();return result};
    wrapped.__codebaseInputV2=true;window.renderWorkspace=wrapped;
  }

  const previousFocusEditor=window.focusEditor;
  window.focusEditor=()=>{
    try{previousFocusEditor?.()}catch{}
    const active=tabs()[0];if(active?.type!=='code')return;
    const cm=editors()[active.id];if(cm)bindEditor(cm,{focus:true});
  };

  window.updatePreviewPort=async()=>{
    compactPreviewChrome();
    const requested=previewIntent;previewIntent=false;
    const input=document.getElementById('preview-port');const iframe=document.getElementById('railway-preview-iframe');
    if(!iframe)return;
    ...
    let sessionId='';
    try{const tab=tabs().find(item=>item?.type==='terminal'&&item?.sessionId);sessionId=String(tab?.sessionId||window.__SULANDRA_CODEBASE_PREVIEW_SESSION__||'').trim()}catch{}
    ...
    if(!token){setPreviewDark('PREVIEW requires an active Sulandra session');return}
  ...
  document.addEventListener('click',event=>{
    ...
    const open=target?.closest?.('#codebase-preview-open,.codebase-preview-open');
    if(open)previewIntent=true;
  },true);

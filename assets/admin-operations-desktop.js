(() => {
  ...
  style.textContent = `
    ...
    @media(max-width:1100px){.ops-folder-grid,.ops-quick-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ops-context-strip{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:650px){.ops-folder-grid,.ops-quick-grid,.ops-context-strip{grid-template-columns:1fr}.ops-hero{padding:22px}.ops-section-head{align-items:start;flex-direction:column}}
  `;
  ...
  function constrainSelector(context) {
    const allowed = new Set(allowedOperatingEntities(context).map((entity) => String(entity.id)));
    const select = document.getElementById('adminCompanySelect');
    if (!select) return;
    [...select.options].forEach((option) => {
      if (option.value && !allowed.has(String(option.value))) option.remove();
    });
    if (!allowed.size) {

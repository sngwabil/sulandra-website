/* IT_AGENT_UI_REGRESSION_REPAIR_V1
   Final browser-only repair for the Administrator IT Agent presentation.
   No authorization, action, persistence, or deployment boundary is changed. */
(()=>{
  'use strict';
  ...
  function restoreStatusBoard(){
    const agent=document.getElementById('agent');
    const shell=qs('.agent-shell',agent);
    const main=qs('.agent-main',agent);
    const view=document.getElementById('itwsActionCenterView');
    ...
    button.setAttribute('aria-label','Open status board');
    button.setAttribute('aria-expanded',drawer.classList.contains('itws-open')?'true':'false');
    ...
    const setOpen=open=>{
      drawer.classList.toggle('itws-open',!!open);
      backdrop.classList.toggle('open',!!open&&compact());
  ...
  function syncComposerClearance(){
    ...
    let keyboardInset=0;
    if(vv&&vv.height<window.innerHeight*.82){
  ...
  function startPendingImage(){
    ...
    window.setTimeout(()=>{
      ...
      pendingImageTimer=setTimeout(()=>{
        if(pendingImageCard?.isConnected){
          const small=qs('small',pendingImageCard);if(small)small.textContent='Image generation is taking longer than expected. The request is still being checked.';
        }
      },45000);
    },160);
  }
  ...
  function artifactId(row){return row?.dataset?.artifact||qs('[data-select-artifact]',row)?.dataset?.selectArtifact||''}
  function artifactName(row){return clean(qs('strong',row)?.textContent)||'Generated artifact'}
  ...
  async function inlineArtifact(row){
    ...
    const card=document.createElement('div');
    card.className='itws-inline-artifact';card.dataset.itwsArtifact=id;
    ...
    if(kind==='image'){
      const loading=document.createElement('div');loading.className='itws-inline-artifact-pending';loading.innerHTML='<span class="itws-progress-spinner"></span><span>Loading generated image…</span>';
      card.append(loading,meta);
    }else{
    ...
    if(kind==='image'&&typeof fetchArtifactBlob==='function'){
      try{
        const result=await fetchArtifactBlob(id,false);
        ...
        qs('.itws-inline-artifact-pending',card)?.replaceWith(img);
      }catch{
        const loading=qs('.itws-inline-artifact-pending',card);if(loading)loading.textContent='Generated image is ready. Use Open to view it.';
      }
    }
  }
  ...
  function installObservers(){
    ...
    if(root){
      new MutationObserver(records=>{
        ...
        if(agentReply){
          removeFallback();
          const text=clean(agentReply.textContent);if(/^IT Agent error:/i.test(text))removePendingImage();
        }
  ...
  function installSendGuard(){
    ...
    send.addEventListener('click',()=>{
      ...
      startFallback();
      if(imageIntent(text))startPendingImage();
      setTimeout(syncComposerClearance,40);
    },true);
  }

(() => {
  ...
  // The legacy browser-scoped settings object is no longer a source of truth.
  // Remove it so an old workstation cannot silently override another company.
  try { localStorage.removeItem(LEGACY_SETTINGS_KEY); } catch {}

  function toast(title, message) {
    const root = $('toast');
    if (!root) return;
    const heading = $('toastTitle');
  ...
  async function request(path, init = {}) {
    ...
    const entityId = String(entity?.id || state.entityId || '');
  ...
  function ensureUi() {
    ...
    const oldSub = module.querySelector('.sub');
    ...
    const companyName = $('settingCompanyName');
    if (companyName) { companyName.required = true; companyName.autocomplete = 'organization'; }
    const companyEmail = $('settingCompanyEmail');
    if (companyEmail) { companyEmail.type = 'email'; companyEmail.autocomplete = 'email'; }
    const companyPhone = $('settingCompanyPhone');
    if (companyPhone) { companyPhone.type = 'tel'; companyPhone.autocomplete = 'tel'; }
    const companyAddress = $('settingCompanyAddress');
    if (companyAddress) companyAddress.autocomplete = 'street-address';
    ...
    if (!$('adminCompanySettingsOperations')) {
      ...
      const saveButton = Array.from(form.querySelectorAll('button')).find((button) => /save settings/i.test(button.textContent || ''));
      if (saveButton) form.insertBefore(operations, saveButton);
    ...
    if (!$('adminCompanySettingsActions')) {
      ...
      if (saveButton) saveButton.after(reload, stateNode);
      else form.append(reload, stateNode);
      ...
      actions.append(saveButton || document.createTextNode(''), reload, stateNode);
      // Move the existing buttons into one managed action row.
      if (saveButton) saveButton.replaceWith(actions);
      else form.appendChild(actions);
      actions.after(note);
    }

    form.querySelectorAll('input,select,textarea').forEach((control) => {
      if (!control.dataset.companySettingsBound) {
  ...
  function setBusy(busy, message) {
    const form = $('adminCompanySettingsForm');
    if (!form) return;
    for (const id of FIELD_IDS) {
      const control = $(id);
      if (control) control.disabled = busy;
    }
    const save = $('adminCompanySettingsSave');
    const reload = $('adminCompanySettingsReload');
  ...
  function renderSource(data) {
    const entityNode = $('adminCompanySettingsEntity');
    const updatedNode = $('adminCompanySettingsUpdated');
    if (entityNode) entityNode.textContent = `${data.code || state.entityCode || 'Company'} — ${data.displayName || state.entityName || 'Selected company'}`;
    const updatedAt = data.settings?.updatedAt || state.updatedAt;
    if (updatedNode) {
      if (updatedAt) {
        const date = new Date(updatedAt);
        updatedNode.textContent = `Loaded from PostgreSQL · Last saved ${Number.isNaN(date.getTime()) ? updatedAt : date.toLocaleString()}`;
      } else {
        updatedNode.textContent = 'Loaded from PostgreSQL · No prior administrative save recorded for this company.';
      }
    }
  }
  ...
  function writeForm(settings = {}) {
    const values = {
      ...
      settingCompanyAddress: settings.companyAddress ?? '',
      settingCompanyPhone: settings.companyPhone ?? '',
      settingCompanyEmail: settings.companyEmail ?? '',
      settingSenderName: settings.senderName ?? '',
      settingUnmonitoredNotice: settings.unmonitoredNotice ?? '',
      settingEmploymentDisclaimer: settings.employmentDisclaimer ?? '',
      settingTimezone: settings.timezone || 'America/New_York',
      settingSupportEmail: settings.supportEmail ?? '',
      settingSupportPhone: settings.supportPhone ?? '',
      settingWebsite: settings.website ?? '',
    };
    Object.entries(values).forEach(([id, value]) => {
      const control = $(id);
      if (control) control.value = String(value ?? '');
    });
    document.querySelectorAll('.hr-tag-preview').forEach((node) => {
      node.textContent = trim(settings.senderName) || 'Human Resources';
    });
  }
  ...
  function validateForm() {
    const form = $('adminCompanySettingsForm');
    if (!form) throw new Error('Company Settings form is unavailable.');
    const companyName = trim($('settingCompanyName')?.value);
    if (!companyName) {
      $('settingCompanyName')?.focus();
      throw new Error('Company name is required.');
    }
    const companyEmail = $('settingCompanyEmail');
    const supportEmail = $('settingSupportEmail');
    const website = $('settingWebsite');
    for (const control of [companyEmail, supportEmail, website]) {
  ...
  function installUnsavedCompanySwitchGuard() {
    ...
    document.addEventListener('change', (event) => {
      ...
      const proceed = window.confirm('You have unsaved Company Settings changes. Switch companies and discard those changes?');
      if (proceed) return;
      event.preventDefault();
  ...
  async function initialize() {
    ...
    window.SulandraCompanySettings = Object.freeze({
      reload: () => loadSettings('manual reload'),
      save: saveSettings,
      state: () => ({ ...state }),
    });
    try {

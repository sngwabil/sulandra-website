...
<script>
window.__sent1=[];window.__sent2=[];
let activeTerminals={term1:{__sulandraWs:{readyState:1,send:data=>window.__sent1.push(String(data))}}};
</script></body></html>`;
...
try{
  ...
  await page.evaluate(()=>{
    ...
    activeTerminals.term2={__sulandraWs:{readyState:1,send:data=>window.__sent2.push(String(data))}};
  });
  await page.waitForTimeout(220);
  ...
  console.log('Codebase iPad keyboard bridge regression passed: tap focus, software input events, Enter/Backspace, hardware keys, multi-terminal isolation, and normal input verified.');
}finally{
  await browser?.close();

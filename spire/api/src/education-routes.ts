    ...
    async (req, res, next) => {
      try {
        ...
        if (courseCodes.length === 0) {
          return res.status(400).json({ error: 'This education package does not contain any courses yet.' });
        }
        ...
        if (employeeIds.length === 0) {
          return res.status(404).json({ error: 'No matching employees were found.' });
        }

...
function loadDockPolish(){if(document.querySelector('script[data-admin-dock-polish]'))return;const script=document.createElement('script');script.src='/admin-dock-polish.js?v=20260804-1';script.async=false;script.setAttribute('data-admin-dock-polish','true');script.onerror=()=>console.error('The compact Sulandra desktop dock could not be loaded.');document.head.appendChild(script);}
function init(){installStyles();const p=loadProfile();applyRhythm(p.rhythm!==false);configureDock();loadDockPolish();document.addEventListener('click',e=>{if(e.target.closest('#adminProfileCustomizationButton,[data-ec-profile]'))setTimeout(injectHeartSetting,80);},true);const observer=new MutationObserver(()=>{configureDock();injectHeartSetting();});observer.observe(document.body,{childList:true,subtree:true});}
